...
The reference value $\mu$ is often $0$, indicating that the 
null hypothesis is that there is no treatment effect. 
...
\begin{eqnarray*}
z = \frac{(\hat\theta - \mu)}{se(\hat\theta)}
\end{eqnarray*}

quantifies how many standard errors the effect estimate $\hat\theta$ is 
away from the reference value $\mu$.
...
In order to calculate a one-sided $P$-value $\tilde p$, the two-sided 
$P$-value is divided by two, so $\tilde p = p/2$ if the effect 
estimate goes to the intended direction, otherwise $\tilde p = 1 - p/2$.
...
The $P$-value is $p \approx `r formatPval(res$p.value)`$, so there
is *strong evidence* against the null hypothesis of no difference between the two drugs.
...
indicates that there evidence of a lower mean birthweight in babies whose 
mother smoked during the pregnancy.

Now let us assume that the alternative hypothesis was that the mean birthweight
is higher in non-smoking as compared to smoking mothers. In that case, 
...
The one-sided $P$-value $\tilde p = `r res.baby2$p.val`$ is half the 
...
In contrast, if the alternative hypothesis 
was that the mean birthweight
is higher in smoking as compared to non-smoking mothers, 
the one-sided $P$-value is
$\tilde p =  1 - p/2 = `r res.baby3$p.val`$.


### Error rates

In contrast to the $P$-value used as an informal quantitative measure 
of the strength of the statistical evidence in significance tests, 
in hypothesis test an "all or nothing" decision-theoretic based on 
error rates is used. This results in a binary classification into
"significant" and "not significant", depending on whether 
the $P$-value lies below or above the significance level $\alpha$, 
see Figure \@ref(fig:strengthofevidence).
$P$-values are uniformly distributed under the null hypothesis $H_0$, 
so have exact Type-I error control.
This means that $\Pr(p < \alpha \mid H_0) = \alpha$ 
...
finding if the *null hypothesis* $H_0$ is true.
...
```{r strengthofevidence, fig.cap = "Significance vs Hypothesis Test, adopted from @bland", echo = F, fig.width=10, fig.height=7}
...
It is useful to compare hypothesis tests to diagnostic tests as discussed in Chapter \@ref(chap:slides1).
If we consider a significant test result as a positive test and equate the null hypothesis of no effect
...
Using Equation \@ref(eq:fprp), the false positive report probability is $`r round(fprp_1, 2)*100`$\%
...
The effect estimate and standard error can be retrieved using
```{r echo = TRUE}
library(biostatUZH)
l <- 78.57
u <- 488.98
...
Using Equation \@ref(eq:pvalCI) adapted to the $t$-distribution, the $P$-value is 
```{r echo = TRUE}
z <- estimate/se
p <- 2*(1 - pt(z, df = 170))
print(p)
```
and corresponds to the output of the $t$-test.
...
For example, in an RCT testing whether an intensive 
...
and is treated as the alternative hypothesis, \ie $H_1: \theta = \Delta$.
...
In the study from @Doyle2024, the minimal clinically relevant
different was $\Delta = -0.5$-percentage point in 
...
Equation \@ref(eq:pow) is now rearranged and gives the required
...
where $v = z_{1 - \beta}$ and *Cohen's $d$* $=\Delta/\sigma$ is the *relative difference* in terms of standard deviations. Formula \@ref(eq:nZtest) is implemented in the function biostatUZH::power.z.test().
...
standard deviation $\sigma$ of the measurements. Table~\@ref(tab:samplesize_cont) gives for this case the required sample size $n=2(u+v)^2$ in each group. The numbers have to be adjusted for different values of $d$. For example, if $d = 1/2$ then the numbers have to be multiplied with 4.
...
:::{.example}
A study is to be carried out in a rural area of Africa to ascertain whether
giving food supplementation during pregnancy increases birth 
weight @KirkwoodSterne (p.418).
A clinically relevant effect of $\Delta = 0.25$kg was selected and a standard
deviation of $\sigma = 0.4$kg was assumed. 
A Type-I error rate of $\alpha = 0.01$ and a power of 
$1 - \beta = 0.95$ were agreed on.
:::


```{r echo=TRUE}
library(biostatUZH)
power.z.test(delta=0.25, sd=0.4, sig.level=0.01, power=.95)
```

```{r echo=FALSE}
n <- power.z.test(delta=0.25, sd=0.4, sig.level=0.01, power=.95)$n
``` 
We need $n = `r ceiling(n)`$ women in each group.

Exact calculations based on the t-test give a slightly larger value for $n$:

```{r echo=TRUE}
power.t.test(delta=0.25, sd = 0.4, sig.level=0.01, power=0.95)
``` 

A possible statement in trial protocol could be:
``A two-sample t-test with two-sided significance level 0.01 will have
95 \% power to detect a mean difference of 0.25kg, given a common 
standard deviation of 0.4kg, when the sample size in each group is 93.''
...
Finally, rearranging Equation \@ref(eq:powbinary) give the 
...
An alternative approach based on the *arcsine square root transformation* (also variance-stabilizing transformation) $h(\pi) = \arcsin(\sqrt{\pi})$ is described in @matthews (Sec.~3.4) where
%
\[
...
:::{.example}
An RCT was conducted to compare surgical biliary bypass and 
endoscopic insertion of a stent for the management of malignant low bile duct 
obstruction [@Smith1994]. The primary outcome was 30 days mortality, and
a mortality drop of $\Delta = 20\% - 5\%  = 15\%$ was considered clinically 
relevant. A power of 95\% and Type-I error rate of 5\% were agreed on. 
:::
...
``` 
A possible statement in trial protocol could be:
"A sample size of `r round(n)` patients in each group will have 95\% power 
to detect a difference in event rate between the two groups with two-sided 
significance level 0.05 when the proportion of patients experiencing the event 
of interest is 0.2 in Group 1 and 0.05 in Group 2."

The alternative approach based on the variance-stabilizing transformation gives a somewhat smaller sample size: 
```{r echo=TRUE}
h <- function(p) asin(sqrt(p))
power.z.test(delta=h(0.2)-h(0.05), sd=0.5, power=.95)
``` 


### Adjusting for loss to follow-up

In practice, the calculated sample size $n$ should be increased to allow for possible non-response or loss to follow-up. If a drop-out rate of $x$ \% can be anticipated, then we obtain the *adjusted sample size*
\[
  n^\star = \underbrace{\frac{100}{100-x}}_{\tiny \mbox{adjustment factor}} \cdot \,\, n
\]
For example, to incorporate a drop-out rate of 20\%, we need to use an adjustment factor of $100/80 = 1.25$.

...
# Some special designs {#specialDesign}

This chapter gives an introduction to some special randomised
controlled trial designs. Cluster-randomised trials no longer
randomise on individual (patient) level.  Equivalence and non-inferiority trials
...
Up to now, we have seen randomisation of patients. *Cluster-randomised trials* allocate groups of patients *en bloc* to the same treatment. 
...
In this study, $41$ practices were randomised to the *status quo* or to 
...
*summary measure* for each cluster and then analyse these summary values.
...
An alternative approach are so-called generalised estimating equations (GEEs). 
...
guidelines and 17 control practices were not sent anything. The outcome 
...
knitr::kable(tableRegression(result, xtable = FALSE),
             caption = "Unweighted linear regression analysis.")
```

```{r echo=FALSE}
knitr::kable(tableRegression(result.w, xtable = FALSE),
             caption = "Weighted linear regression analysis accounting for varying cluster sizes.")
```


#### Logistic regression with random effects {#ch13logisticRand}
...
This is an approximate formula, which assumes fairly large samle sizes and a negligible variation of the cluster size $\bar n_c$.
An improved formula for the design effect is [@Julious] 
\[
D_{\mbox{eff}} = 1+ \rho((1 + {({\CV}_c})^2) \bar n_c-1)
\]
which also takes into account the coefficient of variation ${\CV}_c$ of the cluster sizes, defined as the standard deviation divided by the mean. 
```{r echo=FALSE}
rho <- 0.5
CV <- 0.5
nc <- 7
Deff <- 1+rho*(nc-1)
Deff2 <-  round(1+rho*((1+CV^2)*nc-1), 2)
``` 

For example, suppose $\bar n_c=`r nc`$ and $\rho=`r rho`$, then $D_{\mbox{eff}} = `r Deff`$, so 4-times as many patients are needed than for a standard RCT on patient-level. 
...
equivalence trial, let us suppose that the goal is to show equivalence at a margin of $\delta=0.5$ hours. To make this scenario more reasonable, we simulate 
...
text(max-.5,h-1,"not sig",cex=1.2,font=2,col="blue",pos=4)
...
#text(max-1,h,"not sig",cex=1.2,font=2,col="blue",pos=4)
...
:::{.example #holland}
@holland2017 conducted a randomised, controlled equivalence trial to 
...
The study cohort consists of 458 patients with ARTI. The primary endpoint was the number of days with restrictions due to ARTI. Patients were interviewed 14 days after randomisation and the non-inferiority margin is $\delta = 1$ day. Results
...
The main secondary endpoint was the proportion of antibiotics prescriptions where the PCT arm is hypothesised to be superior. The results were that 58/232 (`r (100*58/232)`\%) patients with PCT-guided therapy and 219/226 (`r round(100*219/226, 0)`\%) patients with standard therapy received antibiotics. This is a substantial reduction of antibiotics use with PCT-guided therapy of `r round(100*(219/226 - 58/232), 0)`\% (95\% CI: 66\% to 78\%).
:::
...
Cluster-randomisation is discussed in @bland (Sections 2.12, 10.13 and 18.8),
equivalence and cluster-randomised trials in @matthews (Sections 11.5--11.6). 
The Statistics Notes 
@SN_clusterRCTs, @SN_clusterAnalysis, @SN_clusterICC, @SN_clusterSS discuss 
different aspects of cluster-randomised trials.

...
Second, patients are allocated to one group or another by *randomisation*.
A more accurate name for an RCT is a randomised *concurrently* controlled
...
Bradford Hill (1897–1991) were pioneers of randomisation.
In the 1920s, R.A. Fisher presented randomisation as a crucial 
...
A.B. Hill also advocated for randomisation [@Hill1937] and was the
trial statistician in one of the very first RCTs among humans: the MRC
Strepromycin trial. However, this trial is not the first study using randomisation and double-blinding. In 1835, more than 100 years before the MRC Streptomycin Trial,
the `Nuremberg salt test' was performed, using randomisation and double blinding
to evaluate the efficacy of homeopathy. @Stolberg2006 summarised and
commented on 
the study.
...
The randomisation series were not known to the 
...
The remaining children were randomised into a 
treatment and a control group with approximately 200'000 children in each 
group, as summarised in Table \@ref(tab:polio).
...
4.  Allocation to treatment by *randomisation*
...
Covid-19. A pivotal trial is designed to provide the critical evidence needed to support regulatory approval of a drug or vaccine, typically in Phase III. 
:::


### Eligibility criteria
A strict set of *eligibility criteria* allows to generalise the
...
trial or may vary from centre to centre.
...
:::{.example}
  ...
  higher than at baseline (98.0 mmHg). This suggests a treatment effect which is
...
```{r senn, fig.cap="Regression to the mean in an example from @senn2011. The left plot shows the joint distribution of diastolic blood pressure measurements at Baseline and at Outcome. The mean of both measurements is 90 mmHg. The right plot shows the truncated distribution of the patients that have been included in the hypothetical trial. The mean DBP at outcome is now more than 2 mmHg higher than at baseline.", echo=FALSE, fig.show="hold", out.width="50%"}
library(knitr)
...
in situations where randomising patients to a control treatment 
...
:::{.example}
Table \@ref(tab:kidney) summarises the results of an observational study
...
```
However, this study is non-randomised, and the distribution of the 
...
Randomisation ensures - in principle - that the 
...
treatment groups. We do not expect this to happen if randomisation is properly
done. However, it is sometimes forgotten that proper randomisation rests on 
...
done as part of a pre-specified method for assessing the data,
...
Randomised controlled trials are the gold standard for assessing the 
...
## ICH E9 guidelines
Founded in 1990, the International Conference on Harmonisation (ICH) strives for harmonisation in regulatory requirements worldwide to ensure that safe, effective and
high-quality medicines are developed. It brings together regulatory authorities and pharmaceutical industry to discuss scientific and technical aspects of
pharmaceuticals. 

The presence of bias may seriously compromise the ability to
draw valid conclusions from clinical trials. Many of the principles outlined in **ICH E9** from 1998 focus on **minimising bias**, see @ICH1998E9.

According to ICH E9, the **sources of bias** discussed earlier in this chapter are referred to as "statistical" bias. The bias introduced through deviations in conduct is referred to as "operational" bias. Some sources of bias arise from the trial design. For example, an assignment of treatments such that patients at lower risk are systematically assigned to one treatment. Other sources of bias arise during the conduct and analysis of a trial. For example, exclusion of patients from analysis based upon knowledge of patient outcomes may affect the accurate assessment of the treatment effect. Therefore, it is important to identify potential sources of bias as completely as possible so that attempts to limit such bias may be made.

At the early stage, we could use **principles of trial design and conduct** to reduce bias.
For example, we could pre-specify objectives, endpoints, and analysis
methods to prevent data-driven choices. We could also ensure blinding by keeping patients, doctors, and sponsors unaware of treatment assignment to avoid conscious or unconscious
influence. Randomisation can also help, by assigning patients to treatments by chance to
balance known and unknown confounding factors. Furthermore, adding control for comparison is helpful for discriminating outcomes caused by the treatment from outcomes caused by other
factors, such as the natural progression of the disease or patient expectations.

There are also something we can do after the trial is closed. We can follow the **principles of trial analysis** to reduce bias. For example, we can control the Type I error probability at a pre-specified threshold, and limit false positive findings to reduce stopping bias. Reliable treatment estimates are important to facilitate regulatory decisions and inform patients and doctors. Intention-to-treat (ITT) can also help to reduce bias, by evaluating the treatment effect on the basis of the intention to treat a patient rather than the actual treatment given. ITT will be discussed more in Chapter \@ref(Ch11).

## Additional references

An introduction to randomised controlled clinical trials is given in @matthews. The design of experiments is discussed in Chapter 2 in @bland. Regression to the mean is discussed in the Statistics Notes @SN_regrtothemean, @SN_exregrtothemean. A practical example for allocation concealment is @adamsen, and @hijazi for convenience sampling.

...
```{r coverage, echo = FALSE, fig.cap = "Empirical coverage of Wald and Wilson confidence intervals with confidence level $95\\%$ and sample size $n=50$ (grey line). The solid line is a local average of the coverage, computed as described in @held", echo = FALSE, message = FALSE }
...
The lower limit of Wald interval is actually negative ("overshoot") and therefore truncated to 0. The lower limit of Agresti's method is also zero. The other methods have a positive lower limit. The method by Clopper-Pearson produces the widest confidence interval. 

...
We also consider the design of studies comparing two diagnostic tests. In an
*unpaired design*, each study subject is tested with only one of the two tests. 
In a *paired design*, each study subject is tested with both tests.
...
can be used. This means that the person who applies the diagnostic test 
must not know the true disease status, and the person who determines 
the true disease status (using a gold standard test)
...
kable(table_data, caption = "2 × 2 table for the Million Women Study.", col.names = c("Mammography", "no", "yes", "Total")) %>%
  kable_styling() %>%
...
In the MW Study (Example \@ref(exm:mws)), the following estimates are obtained 
...
accuracy of screening test for early detection of type 2 diabetes and 
...
The \code{R} function \code{prop.test()} without continuity correction also 
...
This is what the function `epi::epi.tests()` always computes. 
The function `biostatUZH::confIntDiagnostic()` also computes CIs for PPV and NPV for 
...
The accuracy of two diagnostic tests $A$ and $B$ can be compared with ratios of probabilities or ratios of likelihood ratios. These ratios are equal to 1 (the *reference value*), if there is no difference 
...
:::{.example #CVS}
Data from a (hypothetical) randomized unpaired study of chorionic 
villus sampling (CVS) versus early amniocentesis (EA) for the diagnosis 
...
There is some evidence that CVS has a better specificity and LR$^+$ than EA,
...
sensitivity and LR$^{-}$, since the corresponding confidence intervals 
do not include the reference value of 1.

...
negatives, sensisitivy and specificity (with 95\% CI) of 17 studies using HBA1c 6.5\% for detecting diabetes (refer to figure).
:::
           
```{r #diabetes, fig.cap = "Forest plot of HbA1c 6.5\\% for detecting diabetes in @Kaur2020", echo = FALSE}
...
In example \@ref(exm:diabetes) for the first study (Arenata 2010), we obtain
```{r}
wald(x = 58, n = 58 + 87)
wilson(x = 58, n = 58 + 87)
...
In example \@ref(exm:covid) for the viral load cut-off, we get:
...
-  Paired data is best described with separate tables for diseased (Dis) and non-diseased (nonDis) patients.
:::

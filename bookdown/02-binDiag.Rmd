...
negatives, sensisitivy and specificity (with 95\% CI) of 17 studies using HBA1c 6.5\% for detecting diabetes.
:::
...
:::{.example #covid}
[@Schuit2022] conducted a study to assess the performance of Covid-19 Home-Testing Kits [@Schuit2022], see Figure \@ref(fig:schuit).
:::
...
In the MW study (Example \@ref(exm:mws) we obtain for sensitivity:
```{r wald, echo=T}
library(biostatUZH)
(wald(x=629, n=726))
(wilson(x=629, n=726))
...
In example \@ref(exm:diabetes) for the first study (Arenata 2010), we obtain for the specificity
```{r}
wald(x = 763, n = 763 + 25)
wilson(x = 763, n = 763 + 25)
```
...
:::{.example #mws name="continued"}
The predictive values for prevalence $4.5\%$ are:
:::
...
diagnostic tests have been applied to 548 males to detect coronary 
heart disease. The data is paired, as each subjects performs 
both tests.Paired data is best described with separate tables for diseased 
(Dis) and non-diseased (nonDis) patients.
:::
...
```
There is some evidence for differences between the two diagnostic tests in 
Sensitivity and LR$^{-}$,
since the corresponding confidence intervals do not include the reference value 1.

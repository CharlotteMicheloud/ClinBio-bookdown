...
Up to now, we have considered parallel group design. The contrary of that is a *crossover trial*. Such a trial compares the outcome of a patient when given treatment A to the outcome from the *same patient* when given treatment B, so participants act as their own controls.
This will hopefully lead to larger precision of the treatment effect. Crossover trials are possible if the aim of therapy is *not* to cure a condition. A typical application of the crossover design is to compare different pain-relieving drugs. The simplest crossover trial design is the **AB/BA design**.


## The AB/BA design

The AB/BA design is illustrated in Table \@ref(tab:ABBA). There are two treatment periods. Patients are randomized into two sequence groups where


-  patients in group AB receive the treatments in the order A-B, 
-  patients in group BA receive the treatments in the order B-A.
...
a combination of the period and treatment effect. Only the crossover
design ensures that we can disentangle the treatment effect from the
period effect.
...
:::{.example #enuresisex}
The Enuresis Trial is a placebo-controlled trial on $n=`r n`$ children suffering from enuresis (bed wetting). In group AB ($n_{AB}=`r n1`$), the drug is given for 14 days and the outcome (number of dry nights) is recorded. Then, a placebo is administered for a fortnight and the same outcome variable is recorded. In group BA ($n_{BA}=`r n2`$), first placebo and then the drug is given. The outcome is treated as a continuous variable, although strictly speaking it is a count variable with values between 0 and 14.
:::
...
Patient-level data for group AB and group BA comparing the outcomes under treatment and under placebo for each patient are shown in Figure \@ref(fig:enuresis) together with the corresponding means.
...
The mean outcomes are therefore modelled as:
...
# Printing the table using kable
kable(period_data, escape = FALSE, booktabs = TRUE, caption = "Period effects in AB/BA design.")
```
...
:::{.example}
Results of the simple analysis in the Enuresis Trial (Example \@ref(exm:enuresisex)):
:::
...
Just for illustration, butnot recommended: A simpler analysis of the AB/BA design would be to compare differences $d_i$ of treatment to placebo measurements with a paired $t$-test, ignoring group membership:
    \[
...
:::{.example}
Results of a paired $t$-test in the Enuresis Trial (Example \@ref(exm:enuresisex)):
:::
...
:::{.example}
Results in the Enuresis Trial (Example \@ref(exm:enuresisex)):
:::
...
The above analysis assumes that there is no *carryover effect*, \ie effects of the treatment given in period 1 do not persist during period 2. We now describe two statistical approaches to test for carryover in a AB/BA crossover trial with continuous outcome. We emphasize that this is generally not recommended due to lack of power and other problems. The recommended approach is not to use a crossover design when there is a possibility of a carryover effect. You should try to use non-statistical arguments, perhaps based on the half-lives of drugs, etc., to decide how long treatment effects are likely to persist and apply appropriate washout periods between the two treatment periods. 
...
An alternative approach is to use a mixed model with a carryover effect (treatment $\times$ period interaction). However, both methods are heavily affected by between-patient variation (additional $\xi_i$'s would not cancel in the sum $s_i$), so lack power.
...
:::{.example}
Standard test for carryover in the Enuresis Trial (Example \@ref(exm:enuresisex)):
:::
...
:::{.example #cerebro}
In a $2 \times 2$ crossover trial on cerebrovascular deficiency with 67 patients, an active treatment is compared to placebo. The outcome is whether an electrocardiogram was judged normal or abnormal. 
:::
...
For illustration purposes we start with a separate analysis by period. In period 1, each patient receives either the active treatment or the placebo and the same holds for period 2. So, by considering the two periods separately, all observations are independent as we are used to from a parallel group design. 

```{r echo = F}
library(xtable)

tab1 <- table("treatment"=cerebrovascular$treatment[cerebrovascular$time==0], "Period 1"=cerebrovascular$y.f[cerebrovascular$time==0])
tab2 <- table(cerebrovascular$treatment[cerebrovascular$time==1], cerebrovascular$y.f[cerebrovascular$time==1])
...
\begin{eqnarray*}
    \text{OR}_2 &=& \frac{`r tab2[1, 1]` \cdot `r tab2[2, 2]`}{`r tab2[1, 2]` \cdot `r tab2[2, 1]`} \ = \ `r round(est2, 2)` \\
    \text{se(log(OR$_2$))} &=& \sqrt{\tfrac{1}{`r tab2[1, 1]`} + \tfrac{1}{`r tab2[1, 2]`} + \tfrac{1}{`r tab2[2, 1]`} + \tfrac{1}{`r tab2[2, 2]`}} = `r round(se2, 2)`\\
...
The results of a crossover trial with binary outcome can be summarized as in the following table.
...
Only discordant pairs (in red) contribute to estimates of the
...
### Naive analysis using Mc Nemar's test
...
\widehat{\OR} = \frac{\mbox{\scriptsize \# pairs: normal for Active, abnormal for Placebo}}{\mbox{\scriptsize \# pairs: abnormal for Active, normal for Placebo}} = \frac{a + d}{b + c}
...
:::{.example}
Results in the cerebrovascular deficiency trial (Example \@ref(exm:cerebro)):
:::

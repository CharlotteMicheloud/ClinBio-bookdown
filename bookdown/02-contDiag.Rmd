# Continuous diagnostic tests {#doc}
...
### ROC curve
A threshold ("cut-off") $c$ can be used to dichotomise a continuous test result
...
:::{.example #genexpr}
The gene expression data from @pepe with $n=23$ controls (with healthy ovary tissues) and $m=30$ cases of ovarian cancer are shown in Figure \@ref(fig:genexpress). 
...
``` 
We can see that now the diagnostic test is a continuous measurement, and higher value means higher probability of disease. The first column is the gene expression, and the second column contains information about the case and control groups.
```{r echo=TRUE, message=FALSE, warning=FALSE}
head(rocdata, 3)
```


```{r genexpress, echo = FALSE, fig.cap = "Continuous diagnostic test for gene expression.",message=FALSE, warning=FALSE, fig.height=4}
...
The right plot gives a colorised version that indicates the corresponding cut-off value as indicated in the color scale on the right. 
...
```{r ROC-col, fig.cap = "Empirical ROC curve, uncolorised (left) and colorised (right).", echo = FALSE, fig.height=4}
library(ROCR)
pred <- prediction(predictions = rocdata$expr, 
                   labels = rocdata$case)
perf <- performance(pred, "tpr", "fpr")
...
\]
where $\bar D$ is the control, and $D$ is the case. 
...
The $\mbox{AUC}$ can be conveniently computed based on the normalised *Mann-Whitney U-Statistic*:
...
and the bootstrap confidence interval using `pROC::ci.auc()`.
...
diagnostic test is to maximise Youden's
...
In order to maximise Youden's index, a cut-off value of 2.5 ng/ml
...
:::{.definition}
To minimise the overall cost, the *optimal cut-off* is where
...
## Comparing two continuous diagnostic tests
...
respectively `confIntPairedAUCDiff()`.
...
of ROC curves can be found in @pepe (Chapters 4 and 5). 

...
Studies comparing two diagnostic tests can have an unpaired 
...
for sensitivity and specificity are obtained 
...
This is best understood as *Bayesian updating*, the transition from the *pre-test probability* of disease (the prevalence) 
to the *post-test probability* of disease ($\mbox{PPV}$ and $1-\mbox{NPV}$) 
...
Useful tests have $\mbox{LR}^+ > 1$. It then follows that 
$\mbox{LR}^- < 1$, because
\begin{eqnarray*}
&& \mbox{LR}^+ > 1 \\
& \Leftrightarrow & \frac{\mbox{Sens}}{1-\mbox{Spec}} > 1 \\
& \Leftrightarrow & {\mbox{Sens}}  >  {1-\mbox{Spec}} \\
& \Leftrightarrow & {\mbox{Spec}}  >  {1-\mbox{Sens}} \\
& \Leftrightarrow & 1  >  \frac{1-\mbox{Sens}}{\mbox{Spec}} \\
& \Leftrightarrow & \mbox{LR}^-  <  1.
\end{eqnarray*}
...
\end{eqnarray}
So Bayesian updating with odds (rather than probabilities) is particularly simple: We only have to multiply the prior odds
with the relevant likelihood ratio to obtain posterior odds. 
The posterior odds can be easily back-transformed to probabilities as
\[
...
A positive test results in posterior odds of
\[
...
The Wald confidence interval is of *additive* form, because the error term is
...
It is usually better to use the *Wilson* confidence interval for a proportion, in particular 
if $n$ is small or $p$ is close to 0 or 1.
The Wilson interval has better *empirical coverage* than the Wald interval,
as shown in Figure \@ref(fig:coverage) for $n=50$. 
...
```{r coverage, echo = FALSE, fig.cap = "Empirical coverage of Wald and Wilson confidence intervals with nominal confidence level $95\\%$ and sample size $n=50$ (grey line). The solid line is a local average of the coverage, computed as described in @held", echo = FALSE, message = FALSE }
...
Other types of confidence intervals for proportions are available, for example Agresti, 
...
### Likelihood ratios{#sec:CIforLR}
  ...
  is called the 95\% *error factor* (EF). This Wald confidence interval
  is of multiplicative form, as the estimate (the likelihood ratio) is
  multiplied, respectively divided, by the error factor. There are many
  other confidence intervals in biostatistics that are of multiplicative form, for example confidence intervals for risk ratios,
  odds ratios and
  hazard ratios.
...
### Diagnostic odds ratio
  ...
  The confidence interval for the diagnostic odds ratio is also of multiplicative form, again see Table \@ref(tab:notation) for

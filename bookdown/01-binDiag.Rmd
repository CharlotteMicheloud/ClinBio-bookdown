...
\end{eqnarray*}
The negative predictive value NPV is close to 100\%, which means that a women with a negative mammogram is very likely not to have breast cancer. However, the positive predictive value is only `r round(629/4514*100, 1)`\%, so a women with a positive mammogram still has a fairly small probability to have breast cancer. 
...
The diagonstic odds ratio can be calculated as
\[
...
For example, for the screening for depression study (Example \@ref(exm:depression)): 
```{r echo=T}
library(biostatUZH)

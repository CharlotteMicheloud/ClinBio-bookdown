...
A *binary diagnostic test* categorises individuals
...
dichotomised to binary tests using a suitable threshold, see 
Chapter \@ref(doc).

### Study designs{#sec:design}
...
Moreover, studies comparing two tests can have an unpaired 
or a paired design. In an
*unpaired design*, each study subject is tested with only one of the two tests. 
In contrast, each study subject is tested with both tests in a *paired design*.
...
she/he may scrutinize her mammogram more closely for signs of cancer, 
...
```{r prepdata-mammo, echo = FALSE, message = FALSE, warning=FALSE}
  library(biostatUZH)
  library(reporttools)
...
`r format.bigmark(sum(c(tp, fp, fn, tn)))` women between 50 and
64 years underwent a mammography and were followed up for a year to
determine whether or not they have been diagnosed with breast cancer,
confirmed via histology (the gold standard). The results are summarised in Table \@ref(tab:MWS).
:::

```{r MWS, echo = FALSE, warning=FALSE}
library(knitr)
library(kableExtra)
...
simple diagnostic accuracy study with total sample size $n$. For the data from the MW study shown in Table \@ref(tab:MWS)
we have $tp = `r format.bigmark(tp)`$ true positives,  $fp = `r format.bigmark(fp)`$ false positives,  $tn = `r format.bigmark(tn)`$ true negatives,  $fn = `r format.bigmark(fn)`$ false negatives, and a total sample size of 
 $n = `r format.bigmark(n)`$. 
...
kable(table_data, 
      ...
      caption = "Categorisation of test quality based on Youden's index.",
      escape = FALSE)  # escape = FALSE to render LaTeX syntax

```


A perfect test ($J=1$) must have both $\mbox{Sens} = 1$ and $\mbox{Spec} = 1$.
A useless test ($J=0$) has $\mbox{Sens} = 1 - \mbox{Spec}$, 
...
= `r round(sens.ci[[1]]/100+spec.ci[[1]]/100-1, 2)`$, so mammography is a useful test to diagnose breast cancer according to 
Table \@ref(tab:quality). 


Figure \@ref(fig:J-dep) illustrates that the same Youden's index $J$ can be achieved by
very different combinations of sensitivity and specificity. For example, a diagnostic test with sensitivity 0.9 and specificity 0.7 will have the same Youden's index $J=0.6$ as a test with sensitivity 0.7 and specificity 0.9. 

```{r J-dep, fig.cap = "Contour plot of Youden's index as a function of sensitivity and specificity.", echo = FALSE }
sens <- spec <- seq(0.5, 0.99, 0.01) ## c(.5,.6,.7,.8,.9, .95)
J <- matrix(NA, nrow=length(sens), ncol=length(spec))
for(i in 1:length(sens))
    for(j in 1:length(spec))
        J[i,j] <- (sens[i] + spec[j] - 1)
...
##     )))
par(las=1, pty="s")
contour(sens, spec, J, xlab="Sensitivity", ylab="Specificity")
lines(c(0, 0.7), rep(0.9, 2), col="grey", lty=2)
lines(rep(0.7,2), c(0.5, 0.9), col="grey", lty=2)
lines(c(0, 0.9), rep(0.7, 2), col="grey", lty=2)
lines(rep(0.9,2), c(0.5, 0.7), col="grey", lty=2)
##points(0.9, 0.7, pch=19)
##points(0.7, 0.9, pch=19)

               ## key.axes=axis(4, at=seq(0, 4, by = 1), 
...
that is, the probability that a study subject with a negative test 
result actually does not have the disease. 
:::

Predictive values can be directly estimated in a cohort design (see Section
      \@ref(sec:design)), such as the
...
\end{eqnarray*}
The negative predictive value NPV is close to 100\%, which means that a woman with a negative mammogram is very likely not to have breast cancer. However, the positive predictive value is only `r round(629/4514*100, 1)`\%, so a woman with a positive mammogram still has a fairly small probability to have breast cancer. 

This simple calculation assumes that the observed *disease prevalence*
...
Predictive values can also be calculated for other values of the prevalence with Bayes' theorem:
...
after observing a positive, or negative test result.
...
reports a breast cancer prevalence of 4.5\% for 59-year old British women. 
...
kable(table_data, 
      ...
      caption = "Predictive values in the Million Women Study for different prevalences. The entries in the first line are based on the observed disease prevalence in the study sample.",
      escape = FALSE)  # escape = FALSE to render LaTeX syntax
```
...
Likelihood ratios are ratios of probabilities and summarise how many times more (or less) likely
...
$LR$s quantify the *change in disease odds* after having observed a
...
```{r updating, fig.cap = "Bayesian updating with likelihood ratios. For example, the posterior odds after a positive test result are obtained through multiplication of the prior odds with the positive likelihood ratio: $\\frac{1}{21} \\cdot \\frac{27}{1} = \\frac{27}{21}$.", echo = FALSE}
...
\begin{eqnarray}
\mbox{DOR}
&=&  \frac{\Pr(Y=1 \given D=1)/\Pr(Y=0 \given D=1)}{\Pr(Y=1 \given D=0)/\Pr(Y=0 \given D=0)} (\#eq:DOR1)\\
&= & \frac{\Pr(D=1 \given Y=1)/\Pr(D=0 \given Y=1)}{\Pr(D=1 \given Y=0)/\Pr(D=0 \given Y=0)} (\#eq:DOR2)
\end{eqnarray}

is a single indicator of test performance.
It is defined as the ratio of the odds $\Pr(Y=1)/\Pr(Y=0)$ of a positive test result in 
individuals with the disease ($D=1$) to the odds of a positive test result in
individuals without the disease ($D=0$), see Equation \@ref(eq:DOR1). 
It can also be written as the ratio of the odds of disease in test positives relative to the odds of disease in test negatives, see Equation \@ref(eq:DOR2). 
A higher DOR indicates better diagnostic
...
The diagnostic odds ratio can be calculated as
\[
...
very different combinations of sensitivity and specificity. For example, a diagnostic test with sensitivity 0.9 and specificity 0.7 will have the same $\mbox{DOR}=21$ as a test with sensitivity 0.7 and specificity 0.9. The same is true for other summary statistics, such as Youden's index, which is $J=0.6$ in both cases.



```{r DOR-dep, fig.cap = "Contour plot of the diagnostic odds ratio DOR as a function of sensitivity and specificity. The dots represent a diagnostic test with sensitivity 0.9 and specificity 0.7 and *vice versa*, where DOR = 21 in both cases.", echo = FALSE }
sens <- spec <- seq(0.5, 0.99, 0.01) ## c(.5,.6,.7,.8,.9, .95)
...
##     )))
par(las=1, pty="s")
contour(sens, spec, (DOR), xlab="Sensitivity", ylab="Specificity", levels=c(1, 3, 10, 30, 100, 300, 1000, 3000, 10000))
lines(c(0, 0.7), rep(0.9, 2), col="grey", lty=2)
lines(rep(0.7,2), c(0.5, 0.9), col="grey", lty=2)
lines(c(0, 0.9), rep(0.7, 2), col="grey", lty=2)
lines(rep(0.9,2), c(0.5, 0.7), col="grey", lty=2)
...
Details on how to construct confidence intervals  can be found in Appendix
\@ref(confint).
...
@arroll investigated the diagnostic accuracy of two verbally asked questions 
for screening for depression in 15 general practices in New Zealand.
They report $tp = 28$ true positives, $fp = 129$ false positives, 
$tn = 263$ true negatives and $fn = 1$ false negative among $n=421$ 
patients in primary care. Overall, 37\% (157/421) of the patients screened 
positive for depression while only 7\% (29/421) actually had the disease, 
according to a subsequent computerised composite international diagnostic 
interview considered to be the gold standard. 

Sensitivity was 
97\% (95% confidence
interval, 83% to 99%) and specificity was 67% (62% to 72%). 
The likelihood ratio for a positive test
was 2.9 (2.5 to 3.4) and the likelihood ratio for a
negative test was 0.05 (0.01 to 0.35). 
:::
...
```{r depression, fig.cap = "Results from screening for depression study [@arroll].", echo = FALSE, out.width="90%", eval = FALSE}
...
```{r echo=TRUE}
level <- c(0.9, 0.95, 0.99) # confidence level
qnorm((1 + level)/2) # factor
```
...
as shown in Figure \@ref(fig:coverage). Empirical coverage refers to the proportion of times a confidence interval actually contains the true parameter value across many repeated samples.
...
```{r coverage, echo = FALSE, fig.cap = "Empirical coverage of Wald and Wilson confidence intervals with nominal confidence level $95\\%$ and sample size $n=50$ (grey line). The solid line is a local average of the coverage, computed as described in @held.", warning = FALSE, message = FALSE}
...
In Example \@ref(exm:diabetes), for the first study (see the first row, Arenata 2010, in Figure \@ref(fig:diabetes)) we obtain the following confidence interval for the specificity: 
```{r, echo = TRUE}
wald(x = 763, n = 763 + 25, conf.level = 0.95)
wilson(x = 763, n = 763 + 25, conf.level = 0.95)
```
The limits of the Wald confidence interval are -- by construction -- symmetric around the observed proportion. The limits of the Wilson interval are shifted slightly to the left. 


#### Other confidence intervals {-}
Other types of confidence intervals can be calculated, for example Agresti, 
Jeffreys or Clopper-Pearson. Here, we compute the different confidence intervals for 
the sensitivity in Example \@ref(exm:depression) (screening for depression) based on both questions. 

```{r echo=FALSE}
res <- confIntProportion(x=28, n=29)
```
```{r echo=TRUE}
confIntProportion(x=28, n=29)
```

The upper limit 
of the Wald confidence interval is actually larger than 1 ("overshoot"): 
\[
p + \mbox{ET}_{95} = `r res$p` + 1.96 \cdot \sqrt{(`r res$p` \cdot (1-`r res$p`) / 29} = `r round(res$p + 1.96*sqrt(res$p*(1-res$p)/29),3)`
\]
and in the output truncated to 1.
The upper limit of Agresti's method is also 1. The other methods have an upper limit smaller than 1. The method by Clopper-Pearson produces the widest confidence interval. It is often called "exact", but this is misleading, as it is known to be conservative with too large coverage, see Figure \@ref(fig:coverage2). 


```{r coverage2, echo = FALSE, fig.cap = "Empirical coverage of Clopper-Pearson confidence intervals with nominal confidence level $95\\%$ and sample size $n=50$ (grey line). The solid line is a local average of the coverage, computed as described in @held.", echo = FALSE, message = FALSE }
 source("code/settings.R")

 par(mfrow=c(1,1), las=1, pty="s")
 # Szenario festlegen:
 n1 = 50
 x1 = 0:n1
 prob = 0.95 # confidence level?
 ...
 ## Clopper-Pearson-Intervalle berechnen
 cp.ci = clopperPearson(x=x1, n=n1)
 cp.lower = cp.ci[,"lower"]
 cp.upper = cp.ci[,"upper"]

 plot.coverage(cbind(cp.lower, cp.upper), ylim = ygrenzen, smooth = 0.05, main="Clopper-Pearson")
...
### Likelihood ratios{#sec:CIforLR}
  ...
  likelihood ratio $\log {\mbox{LR}}^+$, for example, are
  \[
  \log {\mbox{LR}}^+ - 1.96 \cdot \SE(\log {\mbox{LR}}^+) \mbox{ and } \log {\mbox{LR}}^+ + 1.96 \cdot \SE(\log {\mbox{LR}}^+).
  \]
...
A 95\% confidence interval for ${\mbox{LR}}^-$ is computed accordingly using the standard error $\SE(\log {\mbox{LR}}^-)$ for a negative test result. 
...
For example, for the study on screening for depression (Example \@ref(exm:depression)): 
```{r echo=T}
library(biostatUZH)
confIntDiagnostic(tp=28, fp=129, tn=263, fn=1, cohort=TRUE)
```
...
```{r echo=TRUE, message = FALSE, warning=FALSE}
  ScreeningData <- as.table(matrix(c(28,129,1,263), 
                                   nrow = 2, byrow = TRUE))
  colnames(ScreeningData) <- c("Dis+","Dis-")
  rownames(ScreeningData) <- c("Test+","Test-")
  library(epiR)
  epi.tests(ScreeningData)
```
The apparent prevalence corresponds to $\Pr(Y=1)=37\%$, 
and the true prevalence to $\Pr(D=1)=7\%$.

### Predictive values
 
If the study follows a cohort design, confidence intervals can be
computed for PPV, NPV and disease prevalence directly from the data.  This is what the function `epi::epi.tests()` always computes.  The function `biostatUZH::confIntDiagnostic()` also computes CIs for PPV and NPV for 
...
3.  Confidence intervals for $\mbox{PPV}$ resp. $\mbox{NPV}$ can be obtained by the usual transformation of the limits obtained in step 2. to the  probability scale.



In the screening for depression study (Example \@ref(exm:depression)), the predictive values for a prevalence 
of $10\%$ are:

```{r, echo = TRUE}
confIntDiagnostic(tp=28, fp=129, tn=263, fn=1, pr=.1)
```
The assumed prevalence of 10\% is higher than the prevalence in the data (6.8\%), the positive predictive value is therefore larger and the negative predictive value is smaller. 
...
:::{.example #CVS}
Data from a (hypothetical) randomised unpaired study of 
...
```
 ...
 The sensitivity and specificity are $\mbox{Sens}=116/122=`r round(116/(116+6)*100,1)`$\%
  and $\mbox{Spec}=4844/4878=`r round(4844/(4844+34)*100,1)`$\% for EA and $\mbox{Sens}=111/124=`r round(111/(111+13)*100,1)`$\% and $\mbox{Spec}=4765/4876=`r round(4765/(4765+111)*100,1)`$\% for CVS. 
Hence,
  ...
  \end{eqnarray*}
We see that the sensitivity of EA is `r round(100*(116/(116+6)/(111/(111+13))-1))`\% larger than the sensitivity of
CVS. The specificity of EA is `r round(100*(4844/(4844+34)/(4765/(4765+111))-1))`\% increased compared to
CVS. 
...
with the corresponding confidence intervals, can be computed using the function 
`biostatUZH::confIntIndependentDiagnostic()`: 

```{r echo=FALSE, message=FALSE, warning=FALSE}
confIntIndependentDiagnostic <-  function (tp, fp, tn, fn, names = c(1, 2), conf.level = 0.95, adjust = FALSE)
{
  stopifnot(is.numeric(tp), length(tp) == 2, is.finite(tp),
            is.wholenumber(tp), is.numeric(fp), length(fp) == 2,
            is.finite(fp), is.wholenumber(fp), is.numeric(tn), length(tn) ==
              2, is.finite(tn), is.wholenumber(tn), is.numeric(fn),
            length(fn) == 2, is.finite(fn), is.wholenumber(fn), is.numeric(conf.level),
            length(conf.level) == 1, is.finite(conf.level), 0 < conf.level,
            conf.level < 1, is.logical(adjust), length(adjust) ==
              1, is.finite(adjust))
  ...
  se.log.DOR <- sqrt(1/tp + 1/fp + 1/tn + 1/fn) # DOR
  ...
  se.log.rDOR <- sqrt(sum(se.log.DOR^2)) # relative DOR
  resultA <- confIntDiagnostic(tp[1], fp[1], tn[1], fn[1])
  resultB <- confIntDiagnostic(tp[2], fp[2], tn[2], fn[2])

  estimate1 <- resultA[1:5, 2]
  estimate2 <- resultB[1:5, 2]
  ratio <- estimate1/estimate2
  FNF <- fn/(fn + tp)
  rFNF <- FNF[1]/FNF[2]
  FPF <- fp/(fp + tn)
  rFPF <- FPF[1]/FPF[2]
  ...
  # Error factor:
  EF <- exp(z * c(se.log.rTPF, se.log.rTNF, se.log.rLRp, se.log.rLRm, se.log.rDOR))
  res <- data.frame(matrix(NA, ncol = 6, nrow = 5))
  colnames(res) <- c("type", paste0("estimate", substr(names[1], 1, 4)), paste0("estimate", substr(names[2], 1, 4)), "ratio", "lower", "upper")
  res[, 1] <- c("Sens", "Spec", "LRplus", "LRminus", "DOR")
  res[, 2] <-  estimate1
  res[, 3] <- estimate2
  res[, 4] <- ratio
  res[, 5] <- ratio/EF
...
heart disease. The data is paired, that is, each subject performs 
both tests.
:::

```{r echo=FALSE}
Dis <- as.table(matrix(c(36, 17, 7, 152), ncol=2, byrow=T))
nonDis <- as.table(matrix(c(290, 10, 11, 25), ncol=2, byrow=T))
colnames(Dis) <- colnames(nonDis) <- c("T1-", "T1+")
rownames(Dis) <- rownames(nonDis) <- c("T2-", "T2+")
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
confIntPairedDiagnostic <- function(diseased, nonDiseased, names = c(1, 2),
                                    conf.level = 0.95, adjust = FALSE)
{
  stopifnot(is.numeric(diseased), is.matrix(diseased), dim(diseased) ==
              c(2, 2), is.finite(diseased), is.wholenumber(diseased),
            diseased > 0, is.numeric(nonDiseased), is.matrix(nonDiseased),
            dim(nonDiseased) == c(2, 2), is.finite(nonDiseased),
            is.wholenumber(nonDiseased), nonDiseased > 0, is.numeric(conf.level),
            length(conf.level) == 1, is.finite(conf.level), 0 < conf.level,
            conf.level < 1, is.logical(adjust), length(adjust) ==
              1, is.finite(adjust))

  sens1 <- colSums(diseased)[2]/sum(colSums(diseased))
  sens2 <- rowSums(diseased)[2]/sum(rowSums(diseased))

  spec1 <- colSums(nonDiseased)[1]/sum(colSums(nonDiseased))
  spec2 <- rowSums(nonDiseased)[1]/sum(rowSums(nonDiseased))

  LR_Plus1 <- sens1/(1 - spec1)
  LR_Plus2 <- sens2/(1 - spec2)

  LR_Minus1 <- (1 - sens1)/spec1
  LR_Minus2 <- (1 - sens2)/spec2

  DOR1 <- LR_Plus1 / LR_Minus1
  DOR2 <- LR_Plus2 / LR_Minus2



  seLogrTPF <- sqrt((sum(diseased) - sum(diag(diseased)))/(rowSums(diseased)[2] *
                                                             colSums(diseased)[2]))
  seLogrFPF <- sqrt((sum(nonDiseased) - sum(diag(nonDiseased)))/(rowSums(nonDiseased)[2] *
                                                                   colSums(nonDiseased)[2]))
  seLogrFNF <- sqrt((sum(diseased) - sum(diag(diseased)))/(rowSums(diseased)[1] *
                                                             colSums(diseased)[1]))
  seLogrTNF <- sqrt((sum(nonDiseased) - sum(diag(nonDiseased)))/(rowSums(nonDiseased)[1] *
                                                                   colSums(nonDiseased)[1]))
  seLogrTPOdds <- sqrt(1/diseased[1,2] + 1/diseased[2,1])
  seLogrTNOdds <- sqrt(1/nonDiseased[1,2] + 1/nonDiseased[2,1])
  seLogrDOR <- sqrt(seLogrTPOdds^2 + seLogrTNOdds^2)
  seLogrLRplus <- sqrt(seLogrTPF^2 + seLogrFPF^2)
  seLogrLRminus <- sqrt(seLogrFNF^2 + seLogrTNF^2)

  estimate1 <- c(sens1, spec1, LR_Plus1, LR_Minus1, DOR1)
  estimate2 <- c(sens2, spec2, LR_Plus2, LR_Minus2, DOR2)
  ratio <- estimate1/estimate2
  ...
  if (adjust)
    conf.level <- sqrt(conf.level)
  z <- qnorm((1 + conf.level)/2)
  EF <- exp(z * c(seLogrTPF, seLogrTNF, seLogrLRplus, seLogrLRminus, seLogrDOR))
  res <- data.frame(matrix(data = NA, ncol = 6, nrow = 5))
  colnames(res) <- c("type", paste0("estimate", substr(names[1], 1, 4)),
                     paste0("estimate", substr(names[2], 1, 4)), "ratio","lower", "upper")
  res[, 1] <- c("Sens", "Spec", "LRplus", "LRminus", "DOR")
  res[, 2] <- estimate1
  res[, 3] <- estimate2
  res[, 4] <- ratio
...
print(confIntPairedDiagnostic(diseased=Dis, nonDiseased=nonDis,
                              names = c("T1", "T2"), conf.level = 0.95))
```

There is some evidence that T1 performs better than T2 in terms of 
sensitivity and $LR^{-}$, 
since the corresponding confidence intervals 
do not include the reference value of 1.

Quite often, in practice, we just use the McNemar test for sensitivity analysis for paired data. 

:::{.example #paired2}
In @Barnell2023, data from 8920 eligible participants are collected for evaluating sensitivity and specificity of two noninvasive tests for colorectal cancer screening. Among 8920 participants, 36 had colorectal cancer. Each participant was tested both by the multi-target stool RNA test (mt-sRNA) and the fecal immunochemical test (FIT), which means, test results are paired within individuals. 
:::

```{r, echo=FALSE}
## data from Barnell et al (JAMA)
CC <- matrix(c(2,0,6,28), ncol=2, byrow=TRUE)
rownames(CC) <- c("RNA-", "RNA+")
colnames(CC) <- c("FIT-", "FIT+")
```
To perform McNemar test, we use the `mcnemar.test` function on the matched data from the two tests. 
```{r, echo=TRUE}
print(CC)
mcnemar.test(CC)
```
In the paper, the authors report the uncorrected $P$-value from the test:
```{r, echo=TRUE}
mcnemar.test(CC, correct=FALSE)
```

A $P$-value of 0.01 suggests that the mt-sRNA test showed significant improvement in sensitivity for colorectal cancer compared with results of the FIT.
...
in Chapter 6 and 10.
@glas_etal2003 discuss properties of the diagnostic odds ratio in detail. 

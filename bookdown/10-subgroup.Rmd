...
there is a subgroup difference. However, this constitutes of an erroneous
...
```{r tamoxifen, fig.cap = "Treatment effects with confidence intervals for each subgroup in the National Surgical Adjuvant Breast and Bowel Project Trial.", echo=F, warning=FALSE, fig.height=3, fig.width=5.5, message=FALSE}
library(gplots) 
...
:::{.example #didgeridoo2}
In addition to the primary endpoint (Epworth scale), the Didgeridoo Study 
...
The problem with multiple outcomes is that the probability that at least one will provide a false significant result is greater than the nominal 
...
For small $k$, the FWER under independence is close to the *Bonferroni bound* $k \cdot \alpha$, which is an upper bound on the FWER obtained by applying the Bonferroni inequality, as shown in Figure \@ref(fig:FWER). 
...
FWER <- 1-(1-alpha)^k
par(las=1)
plot(k, FWER, type="l", ylim=c(0,1), xlab=TeX(r"(Number of outcomes $k$)"), axes=FALSE, lwd=1.5)
axis(2)
box()
val <- c(1, 10, 20, 30, 40, 50)
axis(1, at=val, as.character(val))
text(45, 0.1, TeX(r"($\alpha$ = 0.05)"))
abline(h=0.05, lty=2)
...
If the $p$-values are independent, then the procedure has overall Type-I error rate of $\alpha$. 
In the general case, this procedure is not necessarily conservative, but has Type-I error rate close to $\alpha$ for a wide variation of correlation patterns, in contrast to Bonferroni's method [@matthews]. 
Variations of Simes' method are the @Hochberg1988 and @Hommel1988 method.
...
If all outcomes are continuous, then *Hotelling's $T$-test*, a generalisation 
...
1.  Standardise endpoints separately to a common scale:
\[
...
beeswarm(ep.r ~ group, method = "center", breaks=NA, ylab="Rank", xlab="Groups", pch=16, cex=1.5, xlim = c(0.5, 2.5), xaxt = "n", las = 1, lwd = 1.5, main = "", col="red")
title("Rank of Epworth Score")
axis(1, c(1, 2), (c("Didgeridoo", "Control")), padj = 0.5)

beeswarm(rank.sum ~ group, method = "center", breaks=NA, ylab="Rank sum", xlab="Groups", pch=16, cex=1.5, xlim = c(0.5, 2.5), xaxt = "n", las = 1, lwd = 1.5, main = "", col="red")
title("Rank Sum")
axis(1, c(1, 2), (c("Didgeridoo", "Control")), padj = 0.5)
```

...
Multiplicity issues also arise if more than one outcome are considered
in a randomized trial. Here we distinguish methods that adjust the
outcome-specific $p$-values from multivariate methods, which combine the different outcomes in a single analyses. 

## Subgroup analysis

In a subgroup analysis, the question is whether the treatment effect
...
-  RCTs are usually powered to detect an overall difference between the two treatment groups.  Analyses of subgroups will then have less power to detect
...
-   If subgroups are defined by many variables, the risk of a false positive (significant) finding increases. 
  -  The choice of subgroups is difficult. Subgroup analyses are more credible if they were  {\it a priori} specified before the data were collected rather than {\it post hoc}.
...
# Create the data frame with the table data
df <- data.frame(
  Group = c("Breast-fed", "Breast-fed", "Bottle-fed", "Bottle-fed"),
  Type = rep(c("Supplement", "Placebo"), times = 2),
  n = c(64, 102, 169, 285),
  Mean = c(2.445, 2.408, 2.300, 2.195),
  Var = c(0.0853, 0.0987, 0.0752, 0.1018),
  SE = c(0.0365, 0.0311, 0.0211, 0.0189)
)

# Print the table using kable and kableExtra
kable(df,
      caption = "Summary-level data for the Neonatal Hypocalcemia Trial [@matthews].",
...
```{r hypocalcemiafig, fig.cap = "reatment effects and $p$-values for each subgroup in the Neonatal Hypocalcemia Trial", echo=F, warning=FALSE, fig.height=3, fig.width=5.5}
library(gplots)
...
The estimated treatment effects are shown in Figure \@ref(fig:hypocalcemiafig) together with confidence intervals and $p$-values.



#### Test for interaction
The correct approach to investigate whether there is a subgroup effect 
is a *test for interaction*. Suppose we have two subgroups defined by males (M) and females (F). To investigate if the treatment effect $\theta$ differs between subgroups we consider the
%%     truct a *test of interaction* with
*subgroup difference*
     \[
     \Delta \hat {\theta} = \hat
...
Based on the standard error
     \[
     \SE(\Delta \hat {\theta}) = \sqrt{\SE^2(\hat \theta^{\tiny{\mbox{M}}})+\SE^2(\hat \theta^{\tiny{\mbox{F}}})},
     \]
tests and confidence intervals can be constructed.

The test for interaction is known in meta-analysis as the
$Q$-test for heterogeneity and can also be used for more than two groups. Alternatively, a regression model with \alert{interaction} between treatment and subgroup could be used, if individual-level data are available.


:::{.example}
The test for interaction can be performed with the 
function printWaldCI() from biostatUZH.
:::
...
In the Neonatal Hypocalcemia trial, there is *no evidence* for a different treatment effect in the two subgroups ($p=0.22$). Perhaps the study sample size and specifically the breast-fed group ($n=`r myn[1]`$) was too small to provide such evidence.
...
ratios tend to be closer to normal then [@SN_interactionrev].



### Selecting subgroups

The risk of *false positive findings* increases with increasing number of subgroup comparisons. For example, if we compare treatment effects in 20 different subgroups, then we would expect one of the analyses to yield a significant result (at $\alpha=5\%$), even if the treatment effect is the same in all subgroups ($H_0$).
%% This result follows from the (approximate) uniform
%% distribution of a (two-sided) $p$-value under $H_0$ 
%% and holds even if the $p$-values are dependent. 
If the subgroups have been selected *before* the data were collected based on biological or clinical reasoning, then positive (significant) findings are more trustworthy than if subgroups have been defined _post hoc_.
...
\begin{eqnarray*}
Q^+ = \sum_{i: \hat \theta_i \geq 0} T_i^2 & \mbox{ and } & 
Q^- = \sum_{i: \hat \theta_i < 0} T_i^2
\end{eqnarray*}

with $T_i=\hat \theta_i/\SE(\hat \theta_i)$ being the test statistics in each subgroup $i$. The reference distribution for $T$ is a mixture of $I-1$ $\chi^2$-distributions, so we use statistical software to calculate a $p$-value, as implemented in biostatUZH::gailSimon().

An alternative method is based on pairwise comparisons of simultaneous confidence intervals [@panwolfe].


:::{.example}
  In the  National Surgical Adjuvant Breast and Bowel Project  Trial @{matthews} (Section 9.2), the outcome was the proportion disease-free survival at 3 years, comparing chemotherapy (PF) only versus chemotherapy plus tamoxifen (PFT) for the treatment of Breast Cancer. The proportion difference $\theta$ is positive if disease free survival is more likely under PF. Four subgroups based on the progesterone receptor status (PR) and age are considered, see Table \@ref(tab:breastcancer) and Figure \@ref(fig:tamoxifen). There appears to be an advantage to PF in one of the four subgroups while PFT seems preferable in the other three subgroups.
:::
...
df <- data.frame(
  Metric = c("Difference in proportions $\\widehat{\\theta}$", 
             "$\\SE(\\widehat{\\theta})$", 
...
kable(df, booktabs = TRUE, escape = FALSE, 
      caption = "Subgroups by age and progesterone receptor status (PR) in the National Surgical Adjuvant Breast and Bowel Project Trial (Matthews, 2023).") %>%
...
```{r tamoxifen, fig.cap = "Treatment effects with confidence intervals and (two-sided) $p$-values for each subgroup in the National Surgical Adjuvant Breast and Bowel Project Trial [@matthews].", echo=F, warning=FALSE, fig.height=3, fig.width=5.5}
library(gplots) 
...
the function \code{meta::metagen()} shows.
For the Gail-Simon test, we obtain 
$T=`r round(sqrt(pGS["statistic"]), 2)`^2=`r round(pGS["statistic"], 2)`$ and 
$p=`r formatPval(pGS["p-value"])`$.
So, there is moderate to strong evidence for a quantitative interaction ($p=`r formatPval(mymeta$pval.Q)`$), but only weak evidence for a qualitative interaction 
($p=`r formatPval(pGS["p-value"])`$).
...
:::{.example}
Additionally to the primary endpoint (Epworth scale), the Didgeridoo Study (see Example \@ref(exm:didgeridoo)) had also secondary endpoints:
:::
...
\[
k \, p_{(1)}  \leq  \alpha , \mbox{ or }       
(k-1) \, p_{(2)}  \leq  \alpha, 
  \ldots   \mbox{ or }
p_{(k)}  \leq  \alpha.
%p_{(j)} \leq \alpha / (k-j+1)
\]

:::{.example} 
In the Didgeridoo Study:
:::
...
*Simes' method* [@Simes1986] is also based on the ordered $p$-values $p_{(1)} \leq \ldots \leq p_{(k)}$. A significant result is obtained, if 
\[
k \, p_{(1)} \leq \alpha, \mbox{ or }  k/2 \, p_{(2)} \leq \alpha ,  \ldots 
\mbox{ or }
p_{(k)} \leq \alpha.
%p_{(j)} \leq \alpha / (k-j+1)
\]
...
Multiplicity issues can also be avoided by considering the whole $k$-dimensional outcome vector simultaneously rather than separately.
#### Hotelling's $T$-test
If all outcomes are continuous, then *Hotelling's $T$-test*, a generalization of the $t$-Test, could be used.
...
\]
Here $\mbox{M}$ and ${\mbox{SD}}$ are mean and standard deviation of
...
Summary scores in the Didgeridoo Study are shown in Figure \@ref(fig:summaryscore).
Below we repeat the analysis. There are slight differences in the results, presumably due to a different handling
  of missing values (there was one patient with missing partner rating information
...
```{r summaryscore, fig.cap="Summary score analysis combining the $z$-scores of all outcomes. Published figure from the Didgeridoo Study.", fig.align='center', out.width='100%'}
knitr::include_graphics("figures/figzscore.pdf")
```

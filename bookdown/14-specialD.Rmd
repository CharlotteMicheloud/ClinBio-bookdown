...
as shown in Appendix \@ref(app:equivalenceT1E).
...
res <- t.test(extra ~ group, data = sleep, paired = TRUE, conf.level = 0.9)
print(res$conf.int)
``` 
...
tost1 <- t.test(extra ~ group, data = sleep, paired = TRUE, mu = -0.5, 
                alternative = "greater", sig.level = 0.05)
print(tost1$p.value)
tost2 <- t.test(extra ~ group, data = sleep, paired = TRUE, mu = 0.5, 
...
Consider the hypotheses $H_0: \abs{\Delta} > \delta$ and $H_1: \abs{\Delta} \leq \delta$. The type I error rate is calculated for $\Delta = \delta$. The power $1-\beta$ is computed at the center $\Delta=0$ of $H_1$. Appendix \@ref(app:equivalenceSS) shows that the required sample size in each group then is:

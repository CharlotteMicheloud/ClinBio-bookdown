# Randomisation and allocation, blinding and placebos
...
Using *simple randomisation* for allocation of $2n$ patients to 
...
kable(df, format = "html", escape = FALSE,row.names = FALSE, caption = "Values of $N_A$ and corresponding $N_{\\max}$.") %>%
...
```{r simple, fig.cap = "Probability for unequal group sizes with simple randomisation and a total sample size of $30$.", echo=F}
...
```{r powerloss, fig.cap = "Loss in power for unequal group sizes in scenario $d=1$, $n_A+n_B=30$ and $\\alpha = 0.05$.", echo=F}
n <- 30
n1 <- c(1:(n-1))
n2 <- n-n1
...
The problem of unbalanced group sizes can be solved by using a form of restricted randomisation with so-called *random permuted blocks* (RPBs). For example, with blocks of length 4, there are the following six different sequences of length 4 that comprise two As and two Bs:
...
kable(data, format = "html", escape = FALSE, col.names = NULL, caption = "Sequences of random permuted blocks with a block length of 4 and 2 groups.") %>%
  kable_styling(full_width = TRUE, position = "center") %>%
  column_spec(1, border_right = TRUE)
```

In this case, the randomisation selects randomly a block for every group of four patients. Block randomisation using RPBs of length 4 ensures that group sizes never differ by more than 2. After every fourth patient, the two treatment groups must have the same size.
...
# Print the table using kable
kable(df, format = "html", escape = FALSE, col.names = NULL, caption = "Sequences of random permuted blocks with a block length 6 and 3 groups.") %>%
  kable_styling(full_width = TRUE, position = "center") %>%
  column_spec(1, border_right = TRUE)
```
...
:::{.example #ulcers}
An RCT was conducted to examine the use of cold atmospheric plasma 
...
the two treatment groups using block randomisation with mixing block sizes 
of $4$. 
:::
...
The following `R` code creates the randomisation list for RBPs with random 
...
Unequal randomisation can provide greater experience of a new treatment and may 
...
### Stratification
*Stratification* is useful when there is an imbalance with respect to prognostic factors. 
...
in each group, all younger than 16 years old. 
...
*Minimisation* aims to balance the groups with respect to each factor,
but not for each combination of the factors. The minimisation algorithm is:
...
The minimisation is deterministic if $p = 1$ and $D \neq 0$.
...
For example, decisions such as whether to transform the outcome variable, 
...
In the RCT from @Mirpour2020, the patients were blinded, and 
the data were collected by a trained physician 
and nurse who were blinded to the randomisation method and the treatment 
...
$P$-value is lost. 
Only very small $P$-values should be reported with the "$<$" symbol, for example,
"$p < 0.0001$".


An example of a good reporting is:

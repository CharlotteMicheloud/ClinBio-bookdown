...
This chapter discusses bias and precision of clinical
measurements. It treats both repeated measurements taken by the same
observer and repeated measurements taken by different observers and
introduces important concepts such as reliability, repeatability and
agreement.

## Precision of clinical measurements

The *precision* with which measurements are being reported depends on the number of *significant figures*. Significant figures are not the same as *decimal places*. The difference is illustrated in Table \@ref(tab:signfigures). The number of significant figures depends on the use to which the number is to be put and on how accurate it is. Round observations to a few significant figures only for *reporting*, after you have done all calculations.
...
### Measurement error
Several measurements of the same quantity on the same subject will not in general be the same. The standard deviation of repeated measurements on the same subject represents the size of *measurement error*. To estimate the size of measurement error, we need a set of *replicate readings*, obtained by measuring each member of a sample of subjects more than once. The *within-subject standard deviation* $s_w$ can then be found by one-way *analysis of variance* (ANOVA).

:::{.example}
The Peak Expiratory Flow Rate (PEFR) data from @{bland} (Chapter 20.2) 
are 17 pairs of readings made by the same observer with a Wright Peak Flow 
...
person (faster = "better"), see Figure \@ref(fig:PEFRdevice).
:::
...
```{r PEFRdevice, fig.cap="Peak expiratory flow meter.", echo = FALSE}
knitr::include_graphics("figures/peak_flow_meter.jpg")
```

One-way ANOVA for PEFR data:
...
The entry \texttt{Residuals Mean Sq} gives the within-subject variance 
...
It is also possible to calculate a confidence interval for $s_w$:
...
is based on the variance $s_d^2$ of the 
differences \texttt{Diff}:
...
work if there are more than two measurements for some subjects.


### Measurement error related to size

Sometimes *measurement error* is related to the *size* of the measurement. 
If the standard deviation is proportional to the measurement, 
then a *logarithmic transformation* of the measurements may be helpful. 
If we have two measurements per subject, the (rank) correlation between

1. the absolute
  value of the difference between two measurements 
2. and the mean of the two measurements

can be used to check whether such a transformation is needed. 
See Figure \@ref(fig:logtransform) and the lab ``Measurement error proportional to the mean''.
...
```{r logtransform, fig.cap = "Logarithmic transformation to remove patterns in measurement error.", echo = FALSE}

par(las = 1, pty = "s", mfrow = c(1,2))

plot(SC$Measurement1, SC$Measurement2, pch = 19, xlab = "Measurement 1",
     ylab = "Measurement 2", cex = 0.8)
abline(0, 1, lty = 2)
title("Original scale")

plot(log10(SC$Measurement1), log10(SC$Measurement2),
     pch = 19, xlab = "Log measurement 1", 
     ylab = "Log measurement 2", cex = 0.8, cex.axis = 0.9)
abline(0, 1, lty = 2)
title("Logarithmic scale (base 10)")
...
```{r PEFR-corr, fig.cap = "Absolute difference against mean of the two PEFR measurements.", echo = FALSE}
par(las=1)
...
abline(coef(lm(y ~ x)), lty=2, col=1)
text(300, 20, 
...
(ctest <- cor.test(x, y, method="spearman"))
``` 

Spearman's rank correlation coefficient ($\rho_S = `r round(ctest$estimate, 2)`$)
is slightly larger than Pearson's ($\rho = `r format(cor(x, y), nsmall=2, digits=2)`$) but
the large $P$-value $p=`r formatPval(ctest$p.value)`$ gives no evidence for 
an association between the mean PEFR measurement and the absolute difference of
the PEFR measurements, so no transformation is required. 
...
two measurements will lie with probability 0.95.
:::

Repeatability is 
  usually computed as $2 \sqrt{2} s_w$ (or $1.96 \sqrt{2} s_w$), where 
  $s_w$ is the standard deviation within subjects.
  Note that  $s_d = \sqrt{2} s_w$ is the standard deviation of $d$.
  In the PERF example we obtain
  $2 \sqrt{2} s_w 
 = `r round(2*sqrt(2)*sigmaw,1)`$ litres/min. \\[2mm]
...
:::{.definition}
*Reliability* is the correlation between pairs of readings.
:::

  
A problem is that the standard Pearson correlation coefficient between the
first and
  second measurement depends on the ordering of pairs.
  To illustrate the problem we change the ordering of some random pairs in the PERF data: 
...
The solution to this problem is the \emph{intraclass correlation
  coefficient} (ICC). ICC is the average correlation among all
...
A convenient way to compute ICC is as follows: 
...
ICC can be interpreted as the *proportion* of the variance of
  ...
  relates to the population, from which the sample was taken. 
    "This method should only be used, if we have a
  *representative sample* of the subjects in whom we are interested" [@bland]

:::{.example}
  PEFR data continued. For illustration we modify the PERF
...
Earlier we considered *repeatability* and *reliability*: How well do replicates from the same observer agree? 
Now we are interested in the *agreement* between different observers (subjects, machines, methods $\ldots$).
...
:::{.example}
$n=`r nrow(glucose)`$ pairs of blood glucose measurements (in mmol/L) from two different devices: Cobas6000 vs. Hitachi912.
:::
...
correlation respectively regression analysis to assess agreement. However, neither of these methods is
useful to assess agreement:

1. Correlation is not useful to assess agreement, because multiplication of measurements from one device with any factor will not change the correlation, so methods that do not agree may show high correlation. 
For example, the correlation between \emph{10 $\cdot$} Cobas6000 and Hitachi912 measurements remains the same: 

```{r echo=T}
(cor.test(10*co, hi))
```

2. A regression slope $b \approx 1$ does also not necessarily mean
...
For example the regression slope  for *10 +* Cobas6000 vs. Hitachi912 measurements remains around 1: 
...
The method of Bland & Altman was introduces in 1986, see Figure
\@ref(fig:BA). It extends the concept of repeatability (see Section
\@ref(sec:repeatability)) to observations from different devices to introduce
*limits of agreement*.  The original publication is one of the
most cited papers in medical research with more than 40'000 citations
by the end of 2018.
...
The Bland-Altman Plot assesses the dependency of measurement error on
the ``true'' value. It extends to concept of repeatability to
measurements from different divices.



- The $x$-axis gives the mean $(x_i + y_i)/2$ of both measurements as an estimate of true value.
- The $y$-axis gives the difference $d_i = x_i - y_i$ of both measurements to
eliminate variability *between* observations. 


We can read off the Bland-Altman Plot: 

-  *Bias*: mean difference $\bar{d}$ (systematic error) incl. confidence interval.
-  The *limits of agreement* (LoA): Approximately 95\% of points will lie between 
...
The standard error of the limits of agreement are
\begin{equation*}
  \SE(\LOAlow) = \SE(\LOAup) \approx  \sqrt{3/n} \, s_d \ .
\end{equation*}

Those can be used to compute 95\% CI for $\LOAlow$ and $\LOAup$.
It can also be used to compute the required sample size $n$ for a fixed width $w$
...
:::{.example}
The Bland-Altman plot for Glucose measurements is shown in Figure \@ref(fig:blandaltman). 
:::

The bias is `r format(ba1$difference.mean, nsmall=2, digits=2)` mmol/L with 95\% CI `r displayCI(ba1$ci.mean, 2)` (left). The limits of agreement are  
...
```{r blandaltman, fig.cap = "Bland-Altman plot for glucose measurements. Left: With confidence interval for the bias. Right: With confidence intervals for the limits of agreement.", echo = FALSE}
...
To answer the question whether Hitachi912 can be replaced with Cobas6000, 
we have to proceed as follows: 

1. Determine a priori what difference $\delta$ is *clinically relevant*.
2.  If the limits of agreement (lower = `r format(ba1$lower.agreement.limit, nsmall=2, digits=2)`, upper = `r format(ba1$upper.agreement.limit, nsmall=2, digits=2)`) (or the corresponding confidence intervals) are within $[-\delta, \delta]$, then the old device can be replaced.
...
:::{.example}
179 patients completed a *Symptom Interpretation Questionnaire* on two occasions three years apart. Then they were classified how they were able to cope with symptoms @KirkwoodSterne (p.~434, Ex.36.3).
:::
...
## print(xtab2, include.rownames = FALSE, include.colnames = TRUE, floating = F, hline.after = c(1, 3), type = "latex", size = "footnotesize", 
## sanitize.text.function = function(x){x}, table.placement = "h!", tabular.environment = "tabular")
...
The {proportion of agreement} is
\begin{eqnarray*}
...
\begin{eqnarray*}
  \pexp %&=& (n_{1+} n_{+1} + n_{2 +} n_{+ 2})/n_{++}^2 \\
    ...
    &=& `r format((a2 + d2) / n2, nsmall = 2, digits = 2)` %% \, \, (`r format(100*(a2 + d2) / n2, nsmall = 0)`\%)
\end{eqnarray*}
...
:::{.definition}
  Cohen's Kappa is the difference $\pobs - \pexp$, standardized with the maximal possible value $1-\pexp$:
:::
...
-  $\kappa \leq \pobs$
-  $\kappa = 1$: Perfect agreement ($\pobs=1$)
-  $\kappa = 0$: Agreement not larger than expected by chance ($\pobs=\pexp$)
-  The smallest possible value of $\kappa$ is $-\pexp/(1-\pexp)$ %$\rightarrow$ less than chance agreement.
...
However, the interpretation shown in Table \@ref(tab:kappaClass) is simplifying, as
...
(5/94 resp. 6/94).
$\rightarrow$ $\kappa$ depends on the marginal distribution of positive
...
Standard errors require data in the ordinary $2 \times 2$ table
    format:
```{r format1, echo=TRUE}
print(symptoms)
``` 
Application of the bootstrap needs the data *on individual level*,
...
A  Wald confidence interval is obtained conveniently as follows: 
...
A 95\% bootstrap CI for Cohen's Kappa is obtained via
...
### Weighted Kappa
An example with more than two categories is shown in Table \@ref(tab:multicat).
...
kable(health, digits=0, 
      caption = "Example from @bland: Physical health of 366 subjects as judged by a health visitor (HV, the columns) and the subject's general practitioners (GP, the rows).",
      format = "markdown",
      booktabs = TRUE)
``` 


As a start we may only the consider the diagonal entries for the assessment of agreement.
This is  *unweighted kappa*:
\begin{eqnarray*}
...
However, agreement for ordinal variables is better based on the \emph{weighted
...
-  Quadratic weights: %% $w_{ij} = 1 - {(i-j)^2}/{(k-1)^2}$:

```{r }
print(round(w(4, method="quadratic"),2))
``` 

-  Linear weights: %% $w_{ij} = 1 - {|i-j|}/{(k-1)}$:
...
:::{.example}
Analysing agreement of the health assessments from Table ref(tab:multicat) with the weighted $\kappa$.
:::
...
The function \texttt{cohen.kappa()}

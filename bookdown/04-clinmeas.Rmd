...
The one-way ANOVA for the PEFR date (Example \@ref(exm:pefr)) is computed:
...
```
and is $s_w = `r round(sigmaw_sod,1)`$ mg.
...
round(confintSD(sd = sigmaw, dof = 17), 1) # for PEFR
round(confintSD(sd = sigmaw_sod, dof = nrow(sodium_f)), 2) # for sodium
...
differences  `Diff`. For the PEFR data (Example \@ref{exm:pefr}), this 
gives
...
is larger than in the sodium example($\rho_S = `r round(ctest_sod$estimate, 2)`$), 
but both has a large $p$-value ($p=`r formatPval(ctest_pefr$p.value)`$ and 
$p=`r formatPval(ctest_sod$p.value)`$, respectively)
...
Repeatability is 
usually computed as $2 \sqrt{2} s_w$ (or $1.96 \sqrt{2} s_w$), with 
$s_w$ is the within-subjects standard deviation. The standard deviation of $d$ 
is 
$s_d = \sqrt{2} s_w$.
In the PERF data (Example \@ref(exm:pefr)) we obtain
$s_d = 2 \sqrt{2} s_w 
 = `r round(2*sqrt(2)*sigmaw,1)`$ litres/min, and in the 
 sodium data (Example \@ref(exm:sodium)), $s_d = `r round(2*sqrt(2)*sigmaw_sod,1)`$
 mg.
...
In contrast, the \emph{intraclass correlation coefficient} (ICC)
...
:::
```{r, echo = TRUE}
# add how to compute between subject variance 
```

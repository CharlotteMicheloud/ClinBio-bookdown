displayCI <- function(ci, digit = 2, unit = "", text = "english"){
    
    ci <- format(round(ci, digit), nsmall = digit, big.mark = "'")#"\\\\,")
    d <- 1
	...
	if (identical(text, "german.short")){string <- paste(ci[i, 1], unit, " bis ", ci[i, 2], unit, sep = "")}
        if (identical(text, "english")){string <- paste("from ", ci[i, 1], unit, " to ", ci[i, 2], unit, sep = "")}
        if (identical(text, "english.short")){string <- paste(ci[i, 1], unit, " to ", ci[i, 2], unit, sep = "")}
        disp.ci[i] <- string
        }
    return(disp.ci)
}

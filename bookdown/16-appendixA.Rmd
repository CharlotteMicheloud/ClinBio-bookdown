...
This section describes fundamental summaries of the distribution of random variables, namely their expectation and variance. For multivariate random variables, the association between the components is summarized by their covariance or correlation. Moreover, we list some useful inequalities.
...
In short, the generalized estimating equations work in the following way: 
...
This is a reparameterization of the exponential correlation model, where $\rho_{jk} = \left\{\alpha^{d_{jk}}\right\}_{j,k}$ and $\alpha=\exp(-\phi)$. This is also the \(\texttt{AR-1}\) correlation structure mentioned earlier.
...
The **score equation** in the ordinary generalized linear model is given by 
\begin{equation}
{S}(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial {\mu}_i}{\partial
\boldsymbol{\beta}}\right) \Var(Y_i)^{-1} (Y_i - \mu_i) = {0}. 
(\#eq:ordscore)
\end{equation}
To increase flexibility, the quasi-likelihood approach considers the generalized estimating equation with some working variance function $\Var^*(Y_i)$.
$$
{S}^*(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial {\mu}_i}{\partial
...
In standard binomial regression with $n_i$ trials, the variance is $\Var(Y_i) = n_i \pi_i (1-\pi_i)$, where $\pi_i$ is the success probability. The quasi-binomial model generalizes this to
$$
...
The score function \@ref(eq:score) differs from the score equation in the ordinary generalized linear model (\@ref(eq:ordscore)) by replacing $\Var(Y_i)^{-1}$ with a weight $\boldsymbol{W}_0$.

The basic idea is to use the multivariate quasi-likelihood approach for non-normal data to take into account correlation between components of $Y_i$. Therefore, we have the **generalized estimating equation**
\begin{equation}
\boldsymbol{S}(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial \boldsymbol{\mu}_i}{\partial
...
## Generalized linear models with random effects {#GLMrand}

Conditional on random effects $\boldsymbol{U}_i \sim \Nor_q(\boldsymbol{0}, \boldsymbol{G})$ , $Y_{ij}$ follows a generalized linear model with mean $$\mu_{ij} = \E(Y_{ij}) = h(\eta_{ij})$$ and linear predictor 
...
Examples of a logistic regression with random effects can be found in \@ref(ch13logisticRand) and \@ref(ch14logisticRand).

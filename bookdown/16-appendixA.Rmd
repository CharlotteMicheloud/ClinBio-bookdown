...
Denote the observations in the non-diabetic group by $y_1,\ldots, y_m$, the mean by $\mu_{\bar{D}}$ and the sample variance of $y_1,\ldots, y_m$ by $s_{\bar{D}}^2$. This group is independent of the diabetic group (unpaired). We are interested in the mean difference $\Delta=\mu_D-\mu_{\bar{D}}$. If the two groups have equal variances (hence equal standard deviations), then:
...
for $\pi_1$ and $\pi_2$. Here, Wilson CIs also have better properties.
...
:::{.example #ischemia}
In a reliability exercise carried out as part of the Multicenter Study of Silent Myocardial Ischemia, $41$ patients were randomly selected from those who had undergone a thallium-$201$ stress test. The $41$ sets of images were classified as normal or ischemic by the core thallium laboratory and, independently, by clinical investigators from different centers who had participated in training sessions to support standardisation.
...
$$
holds, 
so $\E(S^2) = \sigma^2$, i.e. $S^2$ is unbiased for $\sigma^2$.
...
usually $0$. For example, H$_0$ can be the hypothesis that there is no 
treatment effect.
...
\]
here $x_+ = \max\{x, 0\}$. What we illustrated here for $2 \times 2$ tables can also be generalised to more categories.
...
different versions and it can also be generalised to more categories.
...
To prove this result, we first note that the derivative of a survivor function $S(y)$ of
a continuous test result $Y$ is
...
Note that $1 - \beta/2 = \Phi(z_{1-\beta/2})$, therefore
...
for relatively small $\beta$ (say  $\beta \leq 30$ \%). The second term 
in \@ref(eq:eq2App) 
...
## Sample size calculations in equivalence trials 
...
Note that the expectation of a random variable does not necessarily exist. If the integral exists, then $X$ has **finite expectation**; otherwise, we say $X$ has **infinite expectation**.
...
It can be shown that $\widehat{\boldsymbol{\beta}}_{\boldsymbol{W}} = (\boldsymbol{X}^{\top}\boldsymbol{W}\boldsymbol{X})^{-1}\boldsymbol{X}^{\top}\boldsymbol{W}\boldsymbol{Y}$ 
is the weighted least squares estimate of $\boldsymbol{\beta}$, which minimises the weighted least squares criterion $(\boldsymbol{Y}-\boldsymbol{X} \boldsymbol{\beta})^\top \boldsymbol{W}(\boldsymbol{Y}-\boldsymbol{X} \boldsymbol{\beta})$. Given any weight matrix $\boldsymbol{W}$, $\widehat{\boldsymbol{\beta}}_{\boldsymbol{W}}$ is unbiased with $\E(\widehat {\boldsymbol{\beta}}_{\boldsymbol{W}}) = {\boldsymbol{\beta}}$. 

The weight matrix $\boldsymbol{W}$ is block-diagonal, with each block $\boldsymbol{W_0}$ of size $n \times n$ for $n$ time points. Its inverse, $\boldsymbol{W}^{-1}$, is often called **working covariance matrix** to distinguish it from the **true covariance matrix** $\boldsymbol{\Sigma}$. The term "working" refers to the fact that it is an approximation to the true covariance matrix of the outcomes. 
...
For block-diagonal $\boldsymbol{W}$ we obtain 
\begin{equation}
\widehat{\boldsymbol{\beta}}_{\boldsymbol{W}} =  \left(
...
entries $\widehat{\boldsymbol{\Sigma}}_0$. The **robust standard errors** can be extracted from the diagonal using $\sqrt{\diag(\widehat {\boldsymbol R}_{\boldsymbol{W}})}$.
...
In short, the generalised estimating equations work in the following way: 
...
### GLS's
Generalised Least Squares (GLS) is a parametric method for estimating regression parameters when errors are correlated. Different from GEE, GLS uses Maximum Likelihood (\(\texttt{ML}\)) or Restricted Maximum Likelihood (\(\texttt{REML}\)). 
...
and $\rho \in [0,1)$ is an unknown correlation parameter. This is also called the \(\texttt{exchangeable}\) correlation structure.
...
where
\[
 \text{RSS}(\boldsymbol{V}_0) = {(\boldsymbol{y}-\boldsymbol{X}\widehat{\boldsymbol{\beta}})^{\top}\boldsymbol{V}^{-1}(\boldsymbol{y}-\boldsymbol{X}\widehat{\boldsymbol{\beta}})}
\]
are the (weighted) residual sum of squares, with $\widehat{\boldsymbol{\beta}} = \widehat{\boldsymbol{\beta}} (\boldsymbol{V}_0)$ as defined in \@ref(eq:weighted).
...
### General formulation
Suppose there are $p$ covariates and we want to consider $q$ of them as random effects, we can include individual-specific random effects $\boldsymbol{U}_i$ into the model:
\begin{equation}
...
We might want to consider random effects when we want to capture **individual heterogeneity** in the mean response. Random effect model is useful as it induces **marginal correlation** between observations $Y_{ij}$ and $Y_{ik}$ from the same individual (see equation \@ref(eq:marg2)).

**Parameter estimation**

The marginal moments
\begin{align*}
  \E(\boldsymbol{Y}_i) & = \boldsymbol{X}_i \boldsymbol{\beta} (\#eq:marg1), \mbox{ and}\\
...
Suppose observations $Y_{ij}$'s are made at time points $t_{ij}$, which are included in the covariate vector $\boldsymbol{x}_{ij}=(1,t_{ij},\tilde{\boldsymbol{x}}_{ij}^{\top})^{\top}$.
...
$$
where $\boldsymbol{U}_i$ is a 2-dimensional random effect with mean zero and a $2 \times 2$ symmetric covariance matrix $\boldsymbol{G}$.
This is the **random slope** (strictly random intercept and slope) model.

# Generalised linear model
## General formulation
First we provide a brief review of the linear regression model
$$
Y_i = \boldsymbol{x}_i^{\top} \boldsymbol{\beta} + \epsilon_i \, \mbox{ with } \, \epsilon_i \sim \Nor(0, \sigma^2)  \quad i=1,\dots,n,
$$
with explanatory variables $\boldsymbol{x}_i \in \mathbb{R}^p$ and regression coefficients $\boldsymbol{\beta} \in \mathbb{R}^p$, rewritten as $$Y_i \sim
\Nor(\mu_i=\boldsymbol{ x}_i^{\top} \boldsymbol{ \beta}, \sigma^2)$$ with mean $\mu_i$ and variance $\sigma^2$.

This can be generalised in two ways: firstly we can consider other distributions than the normal distribution for the response variable $Y_i$ (for example, Bernoulli, Binomial, Poisson, $\ldots$). Secondly, the mean $\E(Y_i)=\mu_i$ will be linked to the **linear predictor** $\eta_i = \boldsymbol{ x}_i^{\top} \boldsymbol{ \beta}$ using a (monotone) **link function** $g = h^{-1}$:
$$
 g(\mu_i) = \eta_i \, \mbox{ so } \, \mu_i = h(\eta_i).
$$
Notice how this is different from **general** linear models (Appendix \@ref(General)): **general** linear models assume a normal distribution of residuals, while **generalised** linear models relax this assumption and allow for other distributions from the exponential family. A linear model can be viewed as a  **generalised** linear model with normal errors and identity link.

### Logistic regression
Instead of the normality assumption on $Y_i$ in linear models, other distributions of $Y_i$ are allowed for generalisation. 
...
Except for the intercept, we can interpret $\exp(\beta_k), (k=1,\ldots,p)$ as **odds ratio** in this model. Also notice that the variance is a function of the mean: $\Var(Y_i)=\pi_i(1-\pi_i)$.

If covariates $x_i$ are identical for some observations $Y_i$, then 
aggregation to the binomial form $Y_j = \sum_{i} Y_i \sim \Bin(n_j, \pi_j)$ is possible. 

An example of a logistic regression can be found in Section \@ref(ch821).
...
$$g(\mu_i) = \log(\mu_i).$$
As a consequence, $\exp(\beta_k), (k=1,\ldots,p)$ can be interpreted as a multiplicative **rate ratio** for the expected count. It is also important to note that the variance increases with the mean in this model: $\Var(Y_i)=\E[Y_i]=\mu_i$.
...
### Quasi-likelihood
Quasi-likelihood is used to deal with overdispersion, which means that the variance of the outcome is greater than what is assumed by the model. For example, in a Poisson regression model we have the assumption $\Var(Y_i)=\E(Y_i)=\lambda_i$, but a more flexible quasi-likelihood model for the variance of $Y_i$ is  $\Var^*(Y_i)=\phi \cdot \lambda_i$. When $\phi>1$, there is overdispersion.

The **score equation** in the ordinary generalised linear model is given by 
\begin{equation}
{S}(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial {\mu}_i}{\partial
\boldsymbol{\beta}}\right) \Var(Y_i)^{-1} (Y_i - \mu_i) = {0}. 
(\#eq:ordscore)
\end{equation}
To increase flexibility, the quasi-likelihood approach considers the generalised estimating equation with some working variance function $\Var^*(Y_i)$.
$$
{S}^*(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial {\mu}_i}{\partial
\boldsymbol{\beta}}\right) {\Var}^*(Y_i)^{-1} (Y_i - \mu_i). 
$$
Then the estimate $\widehat{\boldsymbol{\beta}}$ is the root of ${S}^*(\boldsymbol{\beta})$. 
...
The quasi-Poisson model relaxes the standard assumption by introducing a dispersion parameter $\phi$, so that
$$
{\Var}^*(Y_i) = \phi \cdot \Var(Y_i), \quad \phi > 0.
$$
This allows the model to handle overdispersion ($\phi > 1$) or underdispersion ($\phi < 1$) in count data, while keeping the mean structure unchanged. Notice that the estimates of $\beta$ are the same from models with and without overdispersion if $\Var^*(Y_i) = \phi \cdot \Var(Y_i)$, but the standard errors of $\hat{\beta}$ will be inflated by $\sqrt{\hat{\phi}}$ and variances will be inflated by $\hat{\phi}$.
...
In standard binomial regression with $n_i$ trials, the variance is $\Var(Y_i) = n_i \pi_i (1-\pi_i)$, where $\pi_i$ is the success probability. The quasi-binomial model generalises this to
$$
{\Var}^*(Y_i) = \phi \cdot n_i \pi_i (1-\pi_i),
$$
again introducing a dispersion parameter $\phi$. This is particularly useful in cases where the observed proportion of successes shows greater variability than expected under the binomial model.
...
Recall in Appendix \@ref(GEE) Equation \@ref(eq:block) the weighted least squares estimate is
$$\widehat{\boldsymbol{\beta}}_{\boldsymbol{W}} = \left( \sum_{i=1}^m \boldsymbol{X}_i^{\top}\boldsymbol{W}_0 \boldsymbol{X}_i \right)^{-1} \sum_{i=1}^m \boldsymbol{X}_i^{\top}\boldsymbol{W}_0 \boldsymbol{Y}_i,$$
when the
**working correlation matrix** $\boldsymbol{W}^{-1}$ is block-diagonal with entries $\boldsymbol{W}_0^{-1}$.
...
The score function \@ref(eq:score) differs from the score equation in the ordinary generalised linear model (\@ref(eq:ordscore)) by replacing $\Var(Y_i)^{-1}$ with a weight $\boldsymbol{W}_0$.

The basic idea is to use the multivariate quasi-likelihood approach for non-normal data to take into account correlation between components of $Y_i$. Therefore, we have the **generalised estimating equation**
\begin{equation}
\boldsymbol{S}(\boldsymbol{\beta}) = \sum_{i=1}^m \left(\frac{\partial \boldsymbol{\mu}_i}{\partial
\boldsymbol{\beta}}\right) \Cov(\boldsymbol{Y}_i)^{-1} (\boldsymbol{Y}_i - \boldsymbol{\mu}_i)=\boldsymbol{0}, 
(\#eq:multiscore)
\end{equation}
where the multivariate $\Cov(\boldsymbol{Y}_i)^{-1}$ allows for dependence and can be replaced with some **working correlation matrix** $\boldsymbol{W}^{-1}$. This approach is called quasi-likelihood because typically no true likelihood function exists with \@ref(eq:multiscore) as the score equation.
...
The estimate $\hat{\boldsymbol{\beta}}$ is **consistent** (for $m \rightarrow \infty$), even if the correlation structure is not correct, which means $\Corr(\boldsymbol{Y}_i) \neq \boldsymbol{W}^{-1}$, while the estimates of $\boldsymbol{\beta}$ are **efficient** if $\Corr(\boldsymbol{Y}_i) \approx \boldsymbol{W}^{-1}$.
...
## Generalised linear models with random effects {#GLMrand}

Conditional on random effects $\boldsymbol{U}_i \sim \Nor_q(\boldsymbol{0}, \boldsymbol{G})$ , $Y_{ij}$ follows a generalised linear model with mean $$\mu_{ij} = \E(Y_{ij}) = h(\eta_{ij})$$ and linear predictor 
...
### Logistic regression model with random intercept {#logisticRand}
...
Examples of a logistic regression with random effects can be found in Sections \@ref(ch13logisticRand) and \@ref(ch14logisticRand).

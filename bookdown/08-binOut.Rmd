...
summarises the results of this study, including 95\% Wilson CIs for the 
...
kable(data, caption = "Results of the APSAC study.", 
      col.names = c("Therapy", "Dead", "Alive", "Total", "Percentage Dead", 
...
these tests is that "there is no difference between groups".  

Both methods give a $P$-value, but no effect measure. The $\chi^2$-test
...
It will give a slightly smaller value of test statistic $\chi^2$ and hence a slightly larger $P$-value.
...
The $\chi^2$-test and Fisher's exact test are applied to the APSAC study using the following `R` code:
:::
...
The formula for the standard error of the log relative risk given below in Section \@ref(sec:effectMeasures) is slightly more accurate and is usually the preferred one.
...
```{r APSACartCIs, fig.cap= "Wald and Wilson CIs for the ARR in the APSAC study (upper plot) and in an example with artificial data illustrating overshoot (lower plot).", echo=F, warning=FALSE, fig.height=2.5}
library(gplots)
...
randomised trials comparing bypass surgery with coronary angioplasty in
...
```{r NNTforest, fig.cap="Forest plot for NNT (benefit = NNTB, harm = NNTH) in a meta-analysis, reproduced from Figure 3 in @altman1998.", fig.align="center", fig.width = 9.5, echo = FALSE}
...
of the log odds ratio will be infinite if at least one of the entries
...
gives a *weighted average* of strata-specific odds ratios. The MH method can also be applied to strata-specific risk ratios. However, adjustment for continuous variables using MH is only possible after suitable categorisation.


### Logistic regression {#ch821}
...
The unadjusted analysis can be performed as follows:
```{r echo=T}
...
knitr::kable(tableRegression(m1, latex = FALSE, xtable = FALSE),
             caption = "Unadjusted analysis (model `m1`).")
```

The strata-specific estimates for small and large plaque sizes are obtained using:
```{r echo=TRUE}
...
knitr::kable(tableRegression(m2Small, latex = FALSE, xtable = FALSE),
             caption = "Stratified analysis on small plaque size (`m2Small`).")
```
...
knitr::kable(tableRegression(m2Large, latex = FALSE, xtable = FALSE),
             caption = "Stratified analysis on large plaque size (`m2Large`).")
```
...
knitr::kable(tableRegression(m3, latex = FALSE, xtable = FALSE),
             caption = "Logistic regression analysis (`m3`).")
```
...
Patients were
randomised to one of the two treatments for the entire study.
The ordered response is the self-assessment of arthritis classified as
(1) poor, (2) fair, or (3) good. Among the 303 randomised patients,
299 were observed at both baseline and follow-up.
:::

Here is how the data at baseline and at follow-up looks like:
...
kable(output, digits = c(0, 1, 0, 1, 0, 1), align = "r", booktabs = TRUE,
      caption = "Comparison of observed and fitted counts for different covariate combination.") %>%
  add_header_above(c(" " = 1, "Poor follow-up" = 2, "Fair follow-up" = 2, "Good follow-up" = 2))

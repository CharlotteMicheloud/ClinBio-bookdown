...
computes the expected number of cases in each cell of the $2 \times 2$ table \@ref(tab:apsac) under the
null hypothesis of no difference. The test statistic is
\begin{eqnarray*}
    \chi^2 &=& \sum_{\mbox{{\scriptsize all
          cells}}}\frac{(\mbox{{\color{blue}observed counts} $-$ {\color{magenta}expected counts}})^2}{\mbox{{\color{magenta}expected counts}}} \label{eq:chianteile}.
\end{eqnarray*}
The value of the test statistic is compared to a $\chi^2$-distribution with one degree of freedom to obtain a $P$-value.
There is a second
version of the $\chi^2$-test which uses a *continuity correction* to remove some bias in small samples:
\begin{eqnarray*}
    \chi^2 &=& \sum_{\mbox{{\scriptsize all
          cells}}}\frac{(\abs{\mbox{{\color{blue}observed counts} $-$ {\color{magenta}expected counts}}} - \frac{1}{2})^2}{\mbox{{\color{magenta}expected counts}}} \label{eq:chianteile}.
\end{eqnarray*}
...
two are close to each other for small risks in both groups, in which
...
Details on the statistical tests for $2 \times 2$ tables are given in @altman, Section 2.7.

...
are also discussed at the end of this chatper. 
...
Note that the function `fisher.test()` also provides an odds ratio (with 95% CI) 
as effect measure. The output from the $\chi^2$-test can also be used to compute
...
Let $\pi_0$ and $\pi_1$ be the "true" risks of death in the control and intervention
...
can be used to calculate a Wald CI for the ARR.
...
Heparin treatment. The formulation $\widehat{\OR}=(a\cdot d) / (b \cdot c)$ 
...
and two stata. Table \@ref(tab:strat) presents a contingency table 
...
Suppose that we have $k = 1, \ldots K$ categories and defined the odds ratio 
...
Patients were 
randomized to one of the two treatments for the entired study. 

...
This chapter discusses the analysis of longitudinal outcomes in randomised clinical trials. 
...
we may employ a **parallel group design**, where every participant is a
member of a certain treatment group, or a **cross-over design**, where
every participant serves as their own control, meaning treatment changes over time. We separate *longitudinal* from *cross-sectional* effects
...
:::{.definition #balanced}
Longitudinal data are called **balanced**, if the same number of observations $n_i=n$ for each individual $i= 1,...,m$ at the same time points $t_{ij}=t_j, j=1,...,n.$
:::
...
A placebo-controlled, randomised trial has been conducted to study the effect of an orally 
...
```{r TLC2, echo=FALSE, fig.cap = "Visualisation of blood lead levels in the treatment and control group.", warning=FALSE, message=FALSE}
...
bones and tissues is mobilised and a new equilibrium is achieved. In
...
using the area under the curve (AUC) to analyze TLC data. AUC is defined as the integral of each
...
```{r AUCplot, echo=FALSE, fig.cap = "Boxplots of Area Under the Curve for the tlc dataset.", warning=FALSE, message=FALSE}
...
### Generalised change score analysis and extended ANCOVA
...
\(\texttt{exchangeable}\): It assumes equal correlations for every pair of observations made from the same individual, no matter how large the distance is. It is also called a "compound symmetry" correlation structure. Examplary correlation structure:
...
The correlation structure is the autocorrelation of an AR(1) progress. Examplary correlation structure:
$$
\begin{pmatrix}
1 & \rho & \rho^2 \\
\rho & 1 & \rho \\
...
\(\texttt{unstructured}\): It assumes different correlations for every pair of observations made from the same individual. Examplary correlation structure:
...
**Generalised Estimating Equations (GEE)** extend generalised linear models to accommodate correlated data by modeling the marginal expectation of the outcomes and specifying a "working" correlation structure. For how the GEE's model works, see Appendix \@ref(GEE).
...
geeUnANCOVAFull <- geeglm(ANCOVAFull, id=id, data = tlclong2,
                          corstr = "unstructured")
```
Table \@ref(tab:geeUnANCOVAFull) summarises the estimates, confidence intervals, and $P$-values of the full model. 
...
Generalised Least Squares (GLS) is a parametric method for estimating regression parameters when errors are correlated. Different from GEE, GLS uses Maximum Likelihood (\(\texttt{ML}\)) or Restricted Maximum Likelihood (\(\texttt{REML}\)). GLS is more powerful for unbalanced data.
...
The **raw residuals** (\(\texttt{type="response"}\)) are calculated using $r_{ij} = y_{ij} - \boldsymbol{x_{ij}^\top \hat{\beta}}$. The **standardised residuals** (\(\texttt{type="pearson"}\)) are obtained by dividing the raw residuals by an estimate of the standard deviation, $\tilde{r}_{ij} = r_{ij} / \hat{\sigma}$. The **normalised residuals** (\(\texttt{type="normalised"}\)) are adjusted to remove correlations, computed as $\tilde{n}_i = \mathbf{V}_0^{-1/2} \mathbf{\tilde{r}}_i$, where $\mathbf{V}_0$ is the estimated correlation matrix of $\mathbf{r_i}=(r_{i1},\ldots,r_{rm})^\top$.  They are used to check for residual correlation. If the assumed correlation structure is correct, then $\tilde{n}_i \sim \Nor(\mathbf{0,I})$.
...
```
In Table \@ref(tab:standard), the standardised residuals still exhibit an increasing variance.
<!--and identical correlation as in Table \@ref(tab:raw).-->
```{r standard, echo=FALSE}
## standardised residuals still exhibit increasing variance and identical correlation as raw residuals
knitr::kable(printVarCor(r2), caption = "Standardised residuals variance(first column) and covariance.", booktabs = TRUE, align = "l")
```
The normalised residuals in Table \@ref(tab:norm) still exhibit an increasing trend in variance with smaller covariances compared to raw and standardised residuals.
```{r norm, echo=FALSE}
## normalised residuals still exhibit increasing variance with small correlations
knitr::kable(printVarCor(r3), caption = "Normalised residuals variance(first column) and covariance.", booktabs = TRUE, align = "l")
```
...
Table \@ref(tab:treateff) summarises the estimated parameters with their standard errors and $P$-values.
...
knitr::kable(table, col.names = c("Variable", "Estimate", "Lower 95% CI", "Upper 95% CI", "$P$-value"),
...
There are generally two approaches for analyzing binary longitudinal data using generalised linear models. The first one is to use **marginal models** (quasi-likelihood, generalised estimating equations), and the other one is using **conditional models** (i.e. models with random effects). While the marginal approach allows explicitly for correlation or association between observations made from the same individual, a conditional model assumes independence given the random effects, which implies marginal correlation. For non-normal data, the interpretations of regression coefficients are different between the two methods: the marginal interpretation is about "population-averages", but the individual/conditional interpretation is "subject-specific". The first section will focus on the marginal model, and the second section will address the conditional model. 

### Generalised estimating equations  
The basic idea of the marginal approach is to use multivariate generalised estimating equation for non-normal data to take into account correlation between components of the response vector $\boldsymbol{Y}_i$ (see Appendix \@ref(GEEs) for more details.)

:::{.example #respiratory}
We will use the dataset \(\texttt{respiratory}\) available in the package \(\texttt{geepack}\), originally from @Koch1989. The data are from a clinical trial of patients with respiratory illness, where 111 patients from two different clinics were randomised to receive either placebo or an active treatment. Patients were
examined at baseline and at four visits during treatment. The \(\texttt{baseline}\) is always the same for the same patient \(\texttt{id}\). The respiratory status (categorised
...
We also consider the approach using generalised mixed-effects models, for example the logistic regression with random effects, for analysing binary longitudinal data. For more details, see Appendix \@ref(logisticRand). 

:::{.example #respiratory name="continued"}
Here we continue to use the \(\texttt{respiratory}\) dataset and fit a generalised linear mixed-effects model with a random intercept. After that we compare it with a marginal model (GEE).
:::
...
knitr::kable(table, align = "l",
             col.names = c("Variable", "Estimate", "Lower 95% CI", "Upper 95% CI", "$P$-value"),
             caption = "Estimated odds ratios from the generalised linear mixed-effects model with a random intercept.")
...
We can also visualise the estimates of random intercepts in a histogram (Figure \@ref(fig:hist2)).
...
hist(ri, ylab="Frequency", xlab="Random intercept", cex=0.75, main = "", col="lightblue3")
```

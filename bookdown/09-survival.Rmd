...
For example, the study might be finished before all patients 
have died, a patient might decide to withdraw from the study, or a
patient might die from some other reason, for example a car accident.
:::

A central assumption in survival analysis is that the censoring time is 
independent of the survival time (non-informative censoring).
...
:::{.example #sakk}
The Chemotherapy study [@Huguenin2004] is a randomised study to compare two treatments 
for head and neck cancer on $`r nrow(sakk)`$ patients: 
A combination of radiotherapy and chemotherapy (RT+CT)
($n=`r sum(sakk$trt == "A")`$) was compared with radiotherapy alone (RT) 
...
patients. The notion of overall survival (OS) denotes survival without any further 
...
kable(sakk.tab[, 1:8], align = "lcccc", 
      caption = "Data from selected patients in the Chemotherapy study. OS is the overall survival time, a $+$ indicates censoring, LK are the lymph nodes and PS is the performance status.", 
...
The *life table* method is a classical approach to estimate the survival function $S(t)$. It is based on a partition of the time axis into time intervals $i=1,\ldots,I$ of typically equal length. 
...
$\log \{-\log \hat S(i)\}$ with appropriate back-transformations. The corresponding standard errors can be found in textbooks,
...
Aggregation into arbitrary time intervals can then be avoided. 
The *Kaplan-Meier estimate* of the survival function uses intervals 
which contain only up to one event, so $d_i \in \{0, 1\}$ (if there are no ties). 
...
the estimated survival probabilities will be biased.
...
Lastly, the event happens at the time specified. This could be a problem,
for example, if the event were recurrence of a tumour which would be detected at
a regular examination (interval censoring).

The results of a survival analysis depend on the choice of time scale: 
...
print(cohort)
```
With this dataset, the three time scales are as follows: 
```{r, fig.cap = "Illustration of survival data across three distinct time scales: time since study entry (left), calendar time (centre), and age (right)."}
...
risk_data <- data.frame(
  "Time scale" = c("Time since study entry", "Calendar time", "Age"),
  "Risk set at first event" = c("{1, 2, 3, 4, 5, **7**, 8, 9, 10}", 
                                "{1, 3, 4, 5, **7**}", 
                                "{1, 3, 4, 5, 6, **7**, 9}"),
  "Risk set at second event" = c("{1, 3, 4, **5**, 8}", 
                                 "{1, 2, 3, 4, **5**, 6, 8, 9, 10}", 
                                 "{1, 3, 4, **5**, 6, 9, 10}"),
  check.names = FALSE
)

knitr::kable(risk_data, 
             caption = "Risk sets at the first and second event times according to three different time scales.",
             booktabs = TRUE,
             escape = FALSE) # escape = FALSE allows the bold **7** to render
```


From Table \@ref(tab:risk), we can see that the risk set depends on the time scale.

Then how do we select a time scale? 
...
`mark.time = TRUE` will show censoring events (see Figure \@ref(fig:KMbiased)).
:::
...
```{r ggsurvplot, echo = TRUE, fig.cap="Kaplan-Meier estimate of the survival curve in the RT+CT group from Example \\@ref(exm:sakk), with risk table.", message=FALSE, warning=FALSE}
library(survminer)
...
One has to be careful about which groups to compare.
...
*Survival by tumour response analysis* in oncology is an example that 
...
non-responders, see [@Anderson1983, @Anderson2008].
...
such a plot with the `R`function `ggminer::ggsurvplot`. In Section \@ref(sec:timeVarying) we will describe methods to incorporate time-varying covariates in the analysis to avoid immortal time bias.
...
\end{equation*}
So the survival function in group A is the survival function in group B to the power of $\mbox{HR}$: 
\begin{equation*}
...
required. This simple estimate is known as the Pike hazard ratio
...
i.e. we have proportional hazards. A *profile likelihood approach* allows one to estimate the log hazard ratios $\boldsymbol{ \beta}$, no matter which form 
...
```{r ch29, echo=FALSE, results = "asis"}
knitr::kable(tableRegression(model.cox0, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval without adjusting for baseline.", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
...
```{r ch37, echo=FALSE, results='asis'}
knitr::kable(tableRegression(model.cox1, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval adjusted for baseline.", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```
...
\end{equation*}
where $\Delta$ is the log hazard ratio to be detected, and $u$, $v$ are the same as the ones defined in Section \@ref(sec:ss).
...
## Time-varying explanatory variables  {#sec:timeVarying}

Time-varying explanatory variables, e.g. exposure yes/no, can be dealt with by

-  splitting the follow-up of each patient into parts with constant covariate values and
-  treating the parts as distinct subjects.

This approach requires accounting for late entries in the analysis, which `R` supports. The approach is valid if changes in explanatory variables are unrelated to the underlying risk of failure. We will now see two examples of time-dependent analyses.

:::{.example}
The PBC study is about treatment of primary biliary cirrhosis (PBC) of the liver. We will use the first 312 cases in this dataset from the `survival` package since they participated in the randomised trial and contain largely complete data. The `status` at endpoint can be 0 = censored, 1 = transplant or 2 = dead.
:::
...
```{r echo=TRUE}
sel <- c("id", "futime", "status", "trt", "age", "sex", "day", "bili", "protime")
head(pbcseq[,sel], 14)
```
...
knitr::kable(tableRegression(fit0, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval (baseline only).", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```
...
knitr::kable(tableRegression(fit1, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval (with covariates).", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```

A time-dependent analysis includes all measurements of the covariates. First, we need to prepare the data accordingly. The data preparation below is adopted from vignette `timedep.pdf` in package `survival`. It therefore also contains the variables `ascites`, `albumin` and `alk.phos` that are not used in the following analysis. 
...
knitr::kable(tableRegression(fit2, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval (time-dependent analysis).", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```


The analysis reveals that -- after taking into account adjustments
for time-varying covariates -- the effect of `trt` is weaker with a HR of
`r round(exp(coef(fit2)[1]),2)` rather than `r round(exp(coef(fit1)[1]),2)` and a larger $P$-value. 


:::{.example}
Survival by tumour response revisited, see Section \@ref(sec:breslow). This example is about survival in patients with advanced lung cancer shown in Figure \@ref(fig:gglungcancerall).
:::
...
We now randomly generate tumour "responders" with a probability of $0.05$ to become a responder at every month within the first year.
```{r echo=TRUE}
head(response, 3)
...
knitr::kable(tableRegression(badcoxfit, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval (bad fit).", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```

There appears to be an effect of tumour response on survival, but it should not have an effect as responders were generated randomly. The analysis is flawed as it violates "Breslow's first rule" and is subject to *time-dependent bias*. More precisely, the bias is that the probability to be a responder increases with time.
...
knitr::kable(tableRegression(goodcoxfit, latex = FALSE, xtable = FALSE),
             caption = "Estimated hazard ratio and confidence interval (correct fit).", 
      booktabs = TRUE, escape = FALSE, caption.placement = "top", label = NA, row.names = FALSE)%>%
  kable_styling(latex_options = c("striped", "hold_position"), full_width = TRUE)
```


In the correct analysis, there is no evidence for an effect of tumour response on survival, as it should be for randomly generated responders.

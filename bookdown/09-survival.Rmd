...
(95\%-CI: from $`r round(dl,1)`$ to $`r round(du,1)`$ years).
Both medians with CI are shown in Table \@ref(tab:mediane) and graphically in Figure \@ref(fig:sakkMedians).
...
For example, the survival curves of the RT and RT+CT groups shown in Figure
\@ref(fig:sakkTwoGroups). Later, we show how to generate 
...
Figure \@ref(fig:survhazard) shows the survival function once for
a constant hazard rate and once for an increasing hazard rate.

...
-  The new treatment is given to a group of patients, the \emph{treated
  group}, and another treatment (standard or placebo treatment) is given to
  another group of patients, the *control group*, at the same time.
-  Patients are allocated to one group or another by *randomization*.

A more accurate name is randomized *concurrently* controlled
*clinical* trial, because patients in the intervention and in the control group obtain 
treatment at the same time. 

There are often more than two treatments being compared in a single trial,
for example a treatment in different doses. However, the essential ideas of RCTs 
become transparent already for the case of two groups.
...
The Streptomycin trial conducted by the Medical Research Council (MRC) in the U.K
in 1946-7 and the results were published in 1948 [@Strepto1948].
It is considered the first RCT, double-blind and placebo-controlled.
The participants were randomly assigned to Streptomycin and bed-rest, 
or bed-rest alone. The randomization series were not known to the 
investigator and the patient did not know whether they were in the 
treatment or control group. 

Results showed efficacy of the bactericidal antibiotic drug Streptomycin 
against tuberculosis, albeit with
minor toxicity and acquired bacterial resistance to the drug.
Today, Streptomycin, in combination with penicillin, is used in a standard antibiotic
...
the `Nuremberg salt test' was performed, using randomization and double blinding to evaluated the efficacy of homeopathy. \citet{Stolberg2006} summarized and commented on 
the study.
...
There are important ethical aspects of RCTs. 
...
:::{.example}
Coehlo conducted an open-label, single-group, phase 3 
...
from a previous trial was chosen. In this example, 
...
:::{.example}
Table \@ref(tab:kidney) summarizes the results from an observational study from @charig about two different treatments of kidney stones. This is a non-randomized study, as can be seen from the different distribution of the two therapies among patients with small resp. large kidney stones ($87:270$ vs. $263:80$). 
:::
...
# Create the table using kable
kable(data, 
      col.names = c("", "Success (yes)", "Success (no)", "Total"), 
      caption = "Results from an observational study about two different treatments of kidney stones [@charig], overall and in two sub-groups.",
      booktabs = TRUE) %>%
  kable_styling(position = "center", full_width = FALSE)
```


The overall odds ratio is

\[\mbox{OR} = \frac{273 \cdot 61}{77 \cdot 289} = 0.75,\]

meaning that Therapy B is apparently more successful than Therapy A. However, 
the odds ratios in the subgroups of patients with small and large kidney stones are

\[ \quad \quad  \quad \mbox{OR} = \frac{81 \cdot 36}{6 \cdot 234} = {2.10} \quad \quad \text{and} \quad \quad
\mbox{OR} = \frac{192 \cdot 25}{71 \cdot 55} = {1.23}.\]

This sub-group analysis reveals that Therapy A is more successful than Therapy B 
...
Allocation concealment means that treatment allocation is revealed only *after* 
patients have been allocated to treatment groups, 
*concealing the allocation sequence* from those selecting participants into 
the study and assigning participants to intervention groups. 
Without allocation concealment it may be possible to subvert the recruitment 
of participants into the study and their allocation to treatment.


## Bias in RCTs
 
@matthews lists the following five types of biases that may occur in RCTs.

### Selection bias
*Selection bias* refers to the difficulty to select a representative sample from the population of interest. 
Two types of sampling methods can be distinguished. 

-  *random sampling* (also called ``probability
sampling''): All population members have an equal probability of being selected into the study.
-  *non-random sampling* (``non-probability sampling''). 

Selection bias will occur, if the study sample is systematically different
from the population it was intended to represent. For example, 
*convenience sampling* involves selecting patients because
  it is convenient and they are easily accessible. Convenience
  sampling constitutes non-random sampling and is prone to selection bias. 
...
adequate *allocation concealment* (see Section \@ref(sec:structure)). 
...
checking of ``significance'' will introduce *stopping bias*, as
illustrated in Figure \@ref(fig:stoppingbias). 

Let's have a look 
at an example: Data with 250 patients
...
on referees and editors of suitable journals.  Papers reporting
...
```{}
TODO: add text of RWE and target trials 
```

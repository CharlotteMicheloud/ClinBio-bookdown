...
knitr::kable(tableRegression(model1, intercept=FALSE, latex = FALSE, xtable = FALSE))
```
...
which confirms the earlier findings based on regression diagnostics. 
...
## Welch Test
print(WelchTest1 <- t.test(f.up ~ treatment, var.equal = FALSE))
...
b <- seq(-xm, xm, by = .5)
hist(diff.permut, xlab="Mean difference", breaks=b, col="white",
     border = histborder, las = 1)
T <- obs.diff
...
lines(c(T, T), c(0,0.09), col = colpal4[1], lwd=lwd.lines + 2, lty =1)
text(obs.diff-0.9, 0.12, adj=0, paste("Observed difference \n = ", as.character(format(obs.diff, nsmall=2, digits=2)), sep=""), col=colpal4[1], cex=1.5)
text(-obs.diff-0.3, 0.1125, adj=0, paste("", as.character(format(-obs.diff, nsmall=2, digits=2)), sep=""), col=colpal4[1], cex=1.5)
pPermute <- mean(abs(obs.diff)<abs(diff.permut))
text(3, 0.2, paste("two-sided\np = ", as.character(round(pPermute,3)), sep=""),
     adj = 0, font = 2, col=colarea)
text(3.8, 0.04, paste("p=", as.character(round(mean(-obs.diff < diff.permut),3)), sep=""),
     adj = 0, font = 2, col=colarea)
text(-4.7, 0.04, paste("one-sided\np=", as.character(round(mean(obs.diff > diff.permut),3)), sep=""),
     adj = 0, font = 2, col=colarea)
...
results <- data.frame(
  Method = c("t-test",
             ...
             "Mann-Whitney test",
             "Permutation test"),
  `p-value` = c(formatPval(tTest2$p.value),
                 formatPval(WelchTest2$p.value),
                 formatPval(BehrensTest2$p.value),
                 formatPval(MWTest2$p.value),
                 formatPval(pPermute)),

...
<!-- \] -->
-  *Behrens Test*, another option that can be derived with Bayesian arguments
and is implemented in `biostatUZH::behrensTest()`. Details are given in  @ArmitageBerryMatthews (Section 4.3) and @BoxTiao1973 (Section 2.5.3). 

-  The *Mann-Whitney Test* is a nonparametric test which gives a $p$-value that is not compatible with 
the CI for the mean difference. Instead, it provides a CI for the *median*
of the difference between a sample from X and a sample from Y. The Mann-Whitney Test only uses the ranks of the measurements, 
so carries less assumptions than the $t$-test, but also has less power. 
...
Yet another option is a permutation test based on a randomization model, see @matthews (Section 7.6) and 
@Ludbrook1998. 

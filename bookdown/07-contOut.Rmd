...
interval. 
The case for binary outcomes is discussed in Chapter \@ref(binOut). 

:::{.example #didgeridoo}
The Didgeridoo study [@puhan]
is a randomised controlled trial with simple randomisation. Patients 
with moderate obstructive sleep apnoea syndrome were randomised to 
...
knitr::kable(tableRegression(model1, intercept=FALSE, 
             ...
             caption = "Regression analysis of `model1`.")
```

The advantages of the regression analysis are that it can easily be 
generalised and that the residuals can be checked.
In the Didgeridoo study, the regression diagnostics indicate a poor 
model fit, with signs of variance heterogeneity: 

```{r, echo = TRUE, fig.cap="Diagnostic plots for `model1`.", fig.height=4}
par(mfrow=c(1,2), pty="s", las=1)
plot(model1, which=1, pch=19, col=treatment)
plot(model1, which=2, pch=19, col=treatment, xlab = "Theoretical quantiles")
```

Bartlett's test can be used to test the homogeneity of variances, 
...
The test indicates strong evidence for variance heterogeneity ($p=0.003$), 
which confirms the earlier findings based on regression
...
<!-- - The permutation test, based on a randomisation model, see @matthews  -->
...
There is no evidence for a difference in follow-up means ($p \approx 0.26$).
...
knitr::kable(tableRegression(model2, intercept=FALSE,
                             latex = FALSE, xtable = FALSE),
             caption = "Regression analysis of change scores from `model2` (change score).")
```

The `offset(x)` command fixes the coefficient of `x` at 1.

The regression diagnostics show a somewhat better model fit: 
```{r, echo=FALSE, fig.cap="Regression diagnostics plots for `model2`.", fig.height=4}
par(mfrow=c(1,2), pty="s", las=1)
plot(model2, which=1, pch=19, col=treatment, add.smooth=FALSE)
plot(model2, which=2, pch=19, col=treatment, xlab = "Theoritical quantiles")
```
...
- Permutation test, which follows the randomisation model approach. 
...
#### Mann-Whitney test {-}
Mann-Whitney test gives a confidence interval for the median of the 
...
Mann-Whitney test has fewer assumptions than the $t$-test, but 
also less power. 
...
permutation test for the change score analysis. Note that in total, 
there are ${25 \choose 14} \approx `r round(choose(25, 14)/1000000, 1)`$ 
million distinct permutations of treatment allocation.
...
# Create a kable
kable(results, 
      caption = "Summary table of sensitivity analysis.",
      align = "lcc", label = NA) %>%
...
Let us now define some notation. The outcome means
...
*Analysis of covariance* (ANCOVA) is an extension to the change score analysis. It allows estimation of the coefficient of `baseline` from data, instead of a fixed value in change score analysis.
...
knitr::kable(tableRegression(model3, intercept = FALSE, 
                             latex = FALSE, xtable = FALSE), caption = "ANCOVA results from `model3`.")
```

The regression diagnostics indicate a good model fit: 
```{r, echo = FALSE, fig.cap="Regression diagnostics plots for `model3` (ANCOVA). ",fig.height=4}
par(mfrow=c(1,2), pty="s", las=1)
plot(model3, which=1, pch=19, col=treatment, add.smooth=FALSE)
plot(model3, which=2, pch=19, col=treatment, xlab = "Theoretical quantiles")
```
...
:::{.example #didgeridoo name="continued"}
We can compare the three different analysis methods in the Didgeridoo study:
:::
...
knitr::kable(tableRegression(model1, intercept=FALSE, 
                             latex = FALSE, xtable = FALSE),
             caption = "Regression analysis of `model1` (follow-up).")
```
...
knitr::kable(tableRegression(model2, intercept=FALSE, 
                             latex = FALSE, xtable = FALSE),
             caption = "Regression analysis of `model2` (change score).")
```
...
knitr::kable(tableRegression(model3, intercept=FALSE, 
                             latex = FALSE, xtable = FALSE),
             caption = "Regression analysis of `model3` (ANCOVA).")
```
...
par(las=1)
matplot(rho, cbind(var1, var2, var3), xlab=TeX(r"(Correlation $\rho$)"), ylab="Variance factor", type="l", lwd=2, lty=1)
...
```{r lsmeans, echo = TRUE, warning=FALSE, message=FALSE}
...
knitr::kable(tableRegression(model4, intercept = FALSE, latex = FALSE, xtable = FALSE),
             caption = "Regression analysis for `model4` (adjust for other variables).")
```

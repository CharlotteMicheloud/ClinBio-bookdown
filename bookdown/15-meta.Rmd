...
techniques to summarise the effect estimates obtained from several
...
meta-analysis is to summarise the results from a number of clinical
...
4. Provide a summary effect estimate. 
...
```{r preeclampsiaMA, fig.cap = "Results with forest plot from package `meta` for a fixed effect analysis in Example \\@ref(exm:preeclampsia). ", echo = FALSE, fig.height=4.5, fig.width=9.75}
library(meta)
meta1 <- metabin(event.e = treatedPre, n.e = treatedTot, 
                 event.c = controlPre, n.c = controlTot, 
                 sm = "OR" , method = "Inverse", studlab = study)
...
```{r, fig.height=4.5, fig.width=11, echo = TRUE, fig.cap="Forest plot for meta-analysis."}
...
```{r, fig.height=4.5, fig.width=11, echo = TRUE, fig.cap="Forest plot for cumulative meta-analysis."}
...
```{r, echo = TRUE, fig.cap="Bubble plot for meta-regression."}
par(las=1)
bubble(mr1, lwd = 2, col.line = "blue", studlab=TRUE, xlim=c(-5, 60),
       cex.studlab=0.7, ylim=c(0.15, 1.8), ylab = "Risk ratio")
```
...
par(las = 1)
funnel(meta1, xlab = "Risk ratio", ylab = "Standard error")
``` 
...
par(las=1)
funnel(mymeta, xlab="Log odds ratio", ylab = "Standard error")
``` 
...
:::{.example #stroke}
Figure \@ref(fig:trimfillStroke) illustrates the trim-and-fill method for 11 studies of the effect of using gangliosides in reducing case fatality and disability in acute ischemic stroke.
:::
...
knitr::include_graphics("figures/fig-trimfill05.png")
```
...
```{r ch10, echo=TRUE, message=FALSE, warning=FALSE}
library(meta) # load the library
gs <- as.matrix(read.table("data/ganglioside.txt", header = TRUE))
...
knitr::include_graphics("figures/fig-trimfill07.png")
```
...
Begg's rank correlation test computes standardised treatment estimates
...
Then, it tests the null hypothesis that *Kendall's rank correlation* 
(Kendqll's $\tau$) between $\theta_i^*$ and $v_i^*$ is zero. This test 
...
Figure \@ref(fig:smokingEgger) shows a visualisation of Egger's weighted regression in the passive smoking example.
  

```{r smokingEgger, fig.cap = "Visualisation of Egger's weighted regression in the passive smoking example.", echo=FALSE}

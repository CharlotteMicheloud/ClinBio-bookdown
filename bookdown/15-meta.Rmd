...
unpublished studies may also be included, if available from platforms
such as clinicaltrials.gov. The goal of a subsequent
meta-analysis is to summarize the results from a number of clinical
studies on the same research question and to integrate the findings. A
...
The studies entering a systematic review should be sufficiently
homogeneous regarding in- and exclusion criteria and should use the
same effect measure. Typical effect measures are the (standardized)
difference in means $\theta$ between treatment groups for continuous
outcomes. For binary/time to event outcomes, relative treatment
...
Consider nine RCTs about incidence of preeclampsia. The reported effect
measure is an odds ratio for diuretic vs. control, 
...
The overall effect is usually presented in the forest plot as in
...
*Cochran's test for heterogeneity*. But the test "lacks power" 
and its usefulness in the context of meta-analysis is limited.

:::{.example}
For the preeclampsia data (Example \@ref(exm:preeclampsia)), this test yields 
$Q = `r round(t.homo, 1)`$ at $n-1=8$ degrees of freedom
($p=`r formatPval(p.homo,digits=1)`$). 
There is strong evidence for heterogeneity between studies. 
...
Figure \@ref(fig:preeclampsiaMA) shows also the results of a random 
effects analysis, together with the estimated Higgins' $I^2$ and 
heterogeneity variance $\tau^2$, in the preeclampsia 
Example \@ref(ex:preeclampsia).
...
nature and direction. Sources of reporting bias are:
...
:::{.example}
Passive smoking: Consider a meta-analysis of 37 studies of the effect of environmental tobacco smoke on the risk of lung cancer in lifetime non-smokers [@hackshaw1998]. Spouses of smokers and non-smokers have been compared in terms of log relative risk. Are the results shown in a funnel plot in Figure \@ref(fig:smokingFunnel) affected by publication bias?
:::
...
Smaller studies tend to show greater effects, so there is some asymmetry.
However, only visual inspection is not enough to decide if it is real bias or chance.
...
:::{.example #stroke}
Figure~\@ref(fig:trimfillStroke) illustrates the trim-and-fill method for 
11 studies of the effect of using gangliosides in reducing case fatality and disability in acute ischaemic stroke.
:::


```{r trimfillStroke, echo=FALSE, fig.cap="Trim-and-fill method in Example~\\@ref(ex:stroke). The black circles represent the 11 observed studies whereas the red studies on the left are imputed, based on symmetrically 'mirroring' the trimmed studies on the right."}
...
:::{.example}
Data from the passive smoking example is illustrated in Figure~\@ref(fig:smoking_funnel2).
Results from the trim-and-fill method are shown in Figure~\@ref(fig:trimfill_smoking). The summary effect with (in red) and without (in black) the results from the imputed studies is shown at the bottom.
:::


```{r smokingFunnel2, fig.cap = "Funnel plot in the passive smoking Example \\@ref(exm:smoking).", echo=FALSE, warning=FALSE}
library(meta)
ps <- read.table("data/passive_smoking.txt", header = TRUE) 
...
data <- data.frame(
  Model = c("Observed", "{\\color{red} Filled}"),
...
```{r trimfillSmoking, echo=FALSE, fig.cap="Trim-and-fill method in the passive smoking Example~\\@ref(ex:smoking).", fig.align='center'}
...
If visual inspection of a funnel plot is not enough, perform a statistical test. 
Popular methods are: Rank correlation method or Egger's test (weighted regression).
...
:::{.example}
Egger's weighted regression can be performed using R as shown below for the passive smoking example:
:::
...
```{r results="asis"}
library(biostatUZH)
knitr::kable(tableRegression(egger1, latex = FALSE))
``` 
...
using the function limitmeta() from the package metasens: 
```{r echo = TRUE}
...
@bland (Chapter 17) and @matthews(Chapter 12)

...
Sensitivity and specificity now depend on the threshold $c$: 
\begin{eqnarray*}
...
individual without the disease in question) and $m$ 
cases (individual with the disease). 
If there are  *no ties*, meaning that each observation has a unique value
with no two observations being equal, the ROC curve is an *increasing step function*  
with vertical jumps of size $1/m$ and horizontal jumps of 
...
knitr::kable(selected_rows, digits = c(2, 1, 1, 1), 
...
The limits of a Wald confidence interval for ${\mbox{AUC}}$ are:
  \begin{equation*}
...
For *paired samples*, a difference formula based on 
...
diagnosis of pancreatic cancer. A paired study design was used.
:::

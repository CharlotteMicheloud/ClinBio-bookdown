...
with no two observations being equal, the ROC curve is an 
...
@deHond2022 note that the variety in AUC labelling systems in the literature is substantial. However, some rough guidance may still be helpful for end-users, see @ref{tab:???}. Figure ??? illustrates how much two normal distributions (one for the cases and one for controls) with unit variance are apart for certain AUC values. 

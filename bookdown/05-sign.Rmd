...
and is based on a normal test statistic. 
Conversely, a confidence interval can be transformed to a $P$-value 
provided that the CI is of Wald-type. 
...
u <- 494.80
gamma <- 0.95
t_gamma <-  qt(p = (1 - gamma)/2, df = nrow(birthwt) - 2, lower.tail = FALSE)

estimate <- (u + l)/2; print(estimate)
se <- (u - l)/(2*t_gamma); print(se)
z_val <- estimate/se
p <- 2*(1 - pt(z_val, df = nrow(birthwt)))
print(p)
```

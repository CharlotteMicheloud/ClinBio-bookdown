...
It is now well understood by most statisticians that different
research questions require different statistical techniques see
@GoodmanRoyall1988, @Royall, and @Goodman2016 for a general discussion. This is
also the case in clinical trials, where the following questions often
emerge:

- How much does the drug reduce the risk of a disease?
- Is there an effect of the drug at all?
- Is there enough evidence on the efficacy of a drug to authorise marketing?

The first question requires computation of the effect size and its
precision, usually represented by a confidence (or credible)
interval. There has been an important movement in medical statistics over the last decades to focus on estimation
of effect sizes together with information on the inherent imprecision due to sampling variability, see @altman. 
The second question requires quantifying the strength of
evidence for an effect. This is commonly done with $P$-values, but
other measures of evidence exist such as likelihood ratios and Bayes
factors.  The third question requires a decision-theoretic
perspectives, where error rates play a prominent role. The concepts of a
*null hypothesis* and *statistical significance* play a crucial rule to answer the second
and third question, but is often misunderstood. This is reflected by 
the fundamental difference between significance and
hypothesis tests, described in the next section.
...
<!-- The estimate of the treatment effect is denoted by $\hat\Delta$. -->
...
<!-- \begin{eqnarray} -->
<!-- (\#eq:hyp) -->
<!-- H_0: \Delta = \mu \mbox{ vs } H_1: \Delta \neq \mu \, . -->
<!-- \end{eqnarray} -->

The null hypothesis is represented by a reference value 
...
<!-- one-sided, namely $H_1: \Delta > 0$, as only differences in a  -->
...
of Fisher's significance test, where they are considered
as a quantitative measure of the strength of the statistical evidence. 
...
shows the replication power, i.e. the probability of obtaining 
a significant result in an identically designed replication study. 

The two methods are fundamentally different approaches
to statistical inference as illustrated in 
Figure \@ref(fig:strengthofevidence).
It has recently been proposed to redefine the region between $0.005$ and $0.05$ 
...
```{r strengthofevidence, fig.cap = "Significance vs Hypothesis Test, adapted from @bland .", echo = F, fig.width=10, fig.height=7, message=FALSE, warning=FALSE}
...
H_0: \Delta = 0 \, ,
\end{eqnarray}
with $\Delta = \mu_1 - \mu_0$ denoting the difference in group means. 
...
\end{eqnarray*}
quantifies how many standard errors the estimate of $\Delta$
is away from the null. 
...
and the $t$-distribution for small sample sizes, 
in which case the test statistic is usually denoted as $t$ rather than $z$.
The $z$-values can be used to compute the $P$-value: the larger 
the $\abs{z}$, the smaller the two-sided $P$-value. 
...
size of the effect matters, not its direction. In `R`, this can 
be calculated with 

```{r, echo = TRUE}
round(p <- 2 * pnorm(0.49, lower.tail = FALSE), 2)
```
...
for(i in c(min(sleep$ID):max(sleep$ID)))
    lines(c(1,2), c(sleep$extra[sleep$group==1 & sleep$ID==i],sleep$extra[sleep$group==2 & sleep$ID==i]), col= scales::alpha("black", 0.5))
...
res <- t.test(drug1, drug2, data = sleep, paired = TRUE)
...
with a low birthweight [@Hosmer1989]. 
...
Figure \@ref(fig:baby) shows the baby birthweight 
...
```{r echo = TRUE}
res.baby <- t.test(bwt ~ smoke, data = birthwt, var.equal = TRUE)
print(res.baby)
```

A $t$-test can be conducted and the resulting 
$P$-value $p = `r formatPval(res.baby$p.value)`$
indicates that there is evidence for a difference in 
the mean birthweights between babies whose 
mother smoked and did not smoke during the pregnancy.

In two-sided tests, differences between treatment groups *in either direction*
provide evidence against the null hypothesis. In contrast, in one-sided tests,
only differences in a *pre-specified* direction provide evidence 
against the null hypothesis.
If the difference goes in the expected direction, 
...
Now let us assume that the researchers expected the mean baby birthweight
to be higher for non-smoking as compared to smoking mothers. In that case, 
one can calculate a *one-sided $P$-value* by adding `alternative = "greater"`, as follows.
:::

```{r echo = TRUE}
res.baby2 <- t.test(bwt ~ smoke, data = birthwt, 
                    alternative = "greater", var.equal = TRUE)
print(res.baby2)
```

The one-sided $P$-value 
$\tilde p = `r biostatUZH::formatPval(res.baby2$p.val, break.eps = 1e-5)`$ is half the 
two-sided $P$-value as the difference in means goes in 
the expected direction. 
```{r echo = TRUE}
res.baby3 <- t.test(bwt~ smoke, data = birthwt, 
                    alternative = "less", var.equal = TRUE)
print(res.baby3)
```

In contrast, if researchers expect the mean baby birthweight
to be higher for smoking as compared to non-smoking mothers, 
the one-sided $P$-value is
$\tilde p =  1 - p/2 \approx `r biostatUZH::formatPval(res.baby3$p.val, break.eps = 1e-5)`$.
...
(two-sided p < 0.05).
Furthermore, it is possible to transform a $P$-value to a confidence interval, 
provided that the effect estimate is known, the exact $P$-value is reported
and is based on a normal test statistic [@altman2]. 
Conversely, a confidence interval can be transformed to a $P$-value 
provided that the CI is of Wald-type [@altman1]. 
...
<!-- \begin{equation*} -->
<!-- \SE(\widehat{\Delta})=\dfrac{u-l}{2z_\gamma} \, , -->
<!-- \end{equation*} -->
...
<!-- \begin{equation} -->
<!-- (\#eq:pvalCI) -->
<!-- p = 2\left\{1-\Phi\left[\hat{\Delta}/\SE(\hat{\Delta})\right]\right\} \,. -->
<!-- \end{equation} -->
...
:::{.example #birth name="continued"}
The 95\% confidence interval for the difference in mean baby birthweight 
between non-smoking and smoking mothers is 
$[`r round(res.baby$conf.int[1], 2)`, 
`r round(res.baby$conf.int[2], 2)`]$. 
The approximate two-sided $P$-value can be retrieved using:
:::
...
l <- 72.76
u <- 494.80
## estimate is in the middle of the confidence intervals
estimate <- (u + l)/2; print(estimate)

## standard error can be retrieved from width of confidence interval
se <- (u - l)/(2*1.96); print(se)

## z-statistic
z_val <- estimate/se
## two-sided p-value
p <- 2*pnorm(z_val, lower.tail=FALSE); print(p)
```
The exact $p$-value based on the $t$-distribution is $p = `r formatPval(res.baby$p.value)`$, so only slightly larger, see Example \@ref(exm:birth). 
...
<!-- The $z$-statistic and standard error $\SE(\hat\Delta)$ are retrieved  -->
<!-- using  -->
...
<!-- \begin{equation} -->
<!-- (\#eq:CIpval) -->
<!-- \left[\hat\Delta - z_\gamma\,\SE(\hat\Delta), \hat\Delta + z_\gamma \,\SE(\hat\Delta)\right] \, . -->
<!-- \end{equation} -->
...
<!-- and the estimate $\hat\Delta = `r round(estimate, 2)`$  -->
...
finding if the *null hypothesis* $H_0$ is true. The Type-I error rate
is *controlled* at level $\alpha$,
if we reject $H_0$ if and only if $p \leq \alpha$.
...
Power computations need a particular value for $H_1$, e.g. 
$H_1:`` \mbox{ true difference is 3.5 units}"$.
...
It is useful to compare hypothesis tests to diagnostic tests discussed in 
Chapter \@ref(binDiag).
...
misinterpretation; the Type-I error does too. Namely, 
  \[
...
Although it is a mathematically trivial result, the
  ...
  *false positive report probability* is very common. 
  ...
  This statement is wrong, and confuses the Type-I error 
...
```{r reppower, fig.cap = "Replication power as a function of the $P$-value.", fig.height=4.5, fig.width=8, echo=FALSE, warning=FALSE, message=FALSE}
...
matplot(p, 100*partA, type="l", lwd=2, xlim=c(0, 0.05), ylim=c(40,100), lty=mylty, ylab="Replication power (in %)", xlab="Two-sided P-value in the original study", axes=FALSE, col=mycol)
...
Having a fixed target for recruitment implies that you 
...
Sample size calculations require the specification of 
...
difference was $\Delta = -0.5$-percentage points in 
...
Suppose the means of two independent *equally sized groups* are to be compared with the $z$-test. 
In order to derive the sample size formula,
four quantities are required: 

-  Clinically relevant difference $\Delta = \mu_1 - \mu_0$
- Common standard deviation $\sigma$ (measurement error)
- Type-I error rate $\alpha \rightarrow u = z_{1 - \alpha/2}$,  where $z_x$ can be calculated by `qnorm(x)`
...
<!-- under the alternative hypothesis $H_1: \Delta = \Delta$?  -->
...
The required sample size $n$ per group is then
...
in terms of standard deviations $\sigma$. Formula \@ref(eq:nZtest) is implemented in the function `biostatUZH::power.z.test()`.
...
Suppose that $d = 1$, so the clinically relevant difference $\Delta$ equals the 
...
df <- data.frame(
  ...
  "$u$" = c(u1, u1, u2, u2),
  "$v$" = c(v1, v2, v1, v2),
  "$(u + v)^2$" = round(c(uv_squared, (u1 + v2)^2, (u2 + v1)^2, (u2 + v2)^2), 1),
  `Group sample size $n$` = sample_size, 
  check.names = FALSE
)

# Print the table using kable
kable(df, format = "html", caption = "Required sample size depending on Type-I and Type-II error rates, assuming Cohen's $d=1$.",
      align = c("r", "r", "r", "r", "r", "r")) 
```


:::{.example #laparo}
A randomised controlled trial is conducted to assess the clinical and 
...
A clinically relevant effect of $0.33$ standard deviations
was selected.
...
From the output, $n = `r ceiling(n)`$ patients are needed in each group.
...
"A  sample  size  of  194  in  each  group  was  needed  to  detect  a  mean  difference  in  area under  the  curve of 0.33 standard deviations derived from the SF-36 bodily pain domain with 90% power and a 5% (two-sided $\alpha$)  significance level."
...
while $\Delta$ relates to the clinical relevance and 
does not depend on the power.
Suppose that the sample size $n$ is calculated to detect $\Delta$ with a certain power
...
the outcome probabilities $\pi_1$ and $\pi_0$ in the two groups 
are needed. The clinically relevant difference
$\Delta = \pi_1 - \pi_0$ is then implicit. No standard deviation 
is required. 
The 
required sample size per group can be calculated using 
`biostatUZH::power.prop.test()`.
...
The required sample size can be calculated using `R`:
```{r echo=TRUE}
power.prop.test(p1=0.15, p2=0.09, sig.level=0.05, power=0.8)
``` 
The statement in the paper is: 
"Assuming a 15% rate of surgical site infections with  placebo, we estimated that enrolling 920 patients would provide 80% power to detect a 40% relative between group difference in the incidence of the primary outcome (i.e., 15% in the placebo group and 9% in the oral ornidazole group), with a 5% two-sided Type-I error.  "
...
was expected, so 430 patients were required. In Example \@ref(exm:prophy), 
...
the standard error of the estimate $\hat \Delta$ is 

\begin{equation}
(\#eq:3a)
\SE(\hat \Delta) = {\sigma \sqrt{2/n}} .
\end{equation}
...
be based on *precision*, i.e. the *width of the confidence interval* of the treatment effect.


#### Difference between two means {-}

Specify the *width* $w \approx 4 \cdot \SE(\hat \Delta)$ of
the 95\% confidence interval for the mean difference $\Delta$. Both groups are assumed to have sample size $n$.

With $\SE(\hat \Delta) = \sigma \sqrt{2/n}$, we obtain for the sample size in each group:
\[
n = 2 \, \frac{\sigma^2}{\SE(\hat \Delta)^2} = 32 \, \frac{\sigma^2}{w^2} \, . 
\]
...
If $\Delta=\pi_1-\pi_o$ is the difference between two proportions $\pi_1$ and $\pi_0$
with unit variance 
$$
\sigma^2 = \{\pi_1 (1-\pi_1) + \pi_0 (1-\pi_0)\}/2,
$$ the required sample size in each group is
\[
n = \frac{\sigma^2}{\SE(\hat \Delta)^2}=  16 \, \frac{\sigma^2}{w^2} \, ,
\]
where $w = 4 \SE(\hat \Delta)$ is the width of the 95% confidence interval 
for $\Delta$.
For example, if $\pi_1=`r p1`$, $\pi_0=`r p0`$ and $w=0.1$, then $n=`r myn`$.

### Sample size re-estimation
There are various assumptions that can impact sample size calculation. For example, higher-than-expected variance may reduce the power to detect treatment effects. We can use sample size re-estimation at interim analysis, for example re-estimating the standard deviation $\sigma$ in internal pilot, or potentially increase sample size to maintain power level. We can also adjust sample size for "blinded" vs. "unblinded" designs (with respect to treatment assignment). 
Other parameters can also be re-estimated, for example the baseline event rate parameter for binary outcomes. For more discussions, see @Chuang-Stein and @Proschan. 

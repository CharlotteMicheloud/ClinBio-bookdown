...
- How much does the drug reduce the risk of disease?
- Is there an effect of the drug at all?
- Is there enough evidence on efficacy of a drug to authorize marketing?
...
The second question requires to quantify the strength of
...
and third queston, but is often misunderstood. This is reflected by 
...
the standard normal distribution for large samples
and the $t$-distribution for small samples, 
...
- Type-I error rate $\alpha \rightarrow u = z_{1 - \alpha/2}$,  where $z_x =$ `qnorm(x)`
...
w <- 0.1
seDelta <- w/4
sigma2 <- p1*(1-p1)+p0*(1-p0)
myn <- ceiling(sigma2/(seDelta^2))
``` 
...
with unit variance 
$$
\sigma^2 = \pi_1 (1-\pi_1) + \pi_0 (1-\pi_0),
$$ the required sample size in each group is
\[
n = \frac{\sigma^2}{\SE(\hat \Delta)^2}=  16 \, \frac{\sigma^2}{w^2} \, ,
\]
where $w = 4 \, \SE(\hat \Delta)$ is the (approximate) width of the 95% confidence interval 
for $\Delta$.
For example, if $\pi_1=`r p1`$, $\pi_0=`r p0`$ and $w=0.1$, then $\sigma^2=`r sigma2`$ and $n=`r myn`$.

...
for *reporting* only, after all the calculations 
have been done. 
...
:::{.example #pefr}
A peak flow meter is a device used to measure the maximal expiration speed of a 
...
:::{.example #sodium}
The TONE study is a randomised controlled trial aimed at determining 
...
measured in urine. 
Here, we focus on the participants for which the two urinary measurements 
...
$s^2_w = s^2_d/2$ (assuming independence between groups).
...
measurements may be helpful, because taking logarithms converts multiplicative 
errors into additive errors, for which the model assumptions are more valid. 
...
$p=`r formatPval(ctest_sod$p.value)`$, respectively). Hence, there is no evidence for an association between the mean measurement and the absolute difference of the measurements, so no transformation is required. 
...
```{r cotinine, fig.cap = "Measurement 1 versus Measurement 2 for the 20 schoolchildren in Example \\@ref(exm:cotinine)."}
SC <- read.table("data/SalivaryCotinine.txt", header = TRUE)

par(las = 1, pty = "s")
plot(SC$Measurement1, SC$Measurement2, pch = 19, xlab = "Measurement 1 (ng/ml)",
     ylab = "Measurement 2 (ng/ml)", cex = 0.8, 
     col = 2)
...
:::{.definition}
*Repeatability coefficient* is the value below which the difference $d$ between two measurements will lie with a certain probability, usually 0.95.
:::
...
:::{.definition}
*Reliability* represents the extent to which measurements can be replicated. Reliability of a measurement is characterised by the correlation coefficient between pairs of readings. 
:::

Pearson correlation coefficient between the
first and second measurement depends on the ordering of pairs, and is therefore not appropriate to assess the reliability 
...
```{r sb, fig.cap = "Modified PERF data with less between-subject variation.", echo = F}
...
plot(ba1$average, ba1$difference, pch = 19, col = 2, xlim = c(0, 37), 
     ylim = c(-5, 5), xlab="Average of measurements", ylab="Difference of measurements")
...
<!-- into "normaliser" and "non-normaliser", related to how they were  -->
<!-- able to cope with symptoms [@KirkwoodSterne, p. 434, Ex.36.3]. -->
<!-- ::: -->
:::{.example #Hazir}
In a study of chest radiography in children in Pakistan (see @Hazir2006), children aged 2 to 59 months with non-severe pneumonia were invited to participate. In total 2000 children were enrolled, for whom 1848 chest radiographs were available for assessment. Two consultant radiologists used standardised criteria to evaluate the chest radiographs, with no clinical information available to them. The primary outcome was diagnosis of pneumonia (absent or
present) from chest radiographs. 
:::
...
## mat <- rbind(c("First classification", "normaliser", "non-normaliser", "Total"), mat)
## mat[2:4, 1] <- c("normaliser", "non-normaliser", "Total")
## dimnames(mat)[[2]] <- c("", "Second classification", "", "")
## xtab2 <- xtable(mat, align = "cc|cc|c", caption = "", label = "tab: kappa1")
radiologists <- rbind(
    matrix(rep(c(1, 1), 176), ncol = 2, byrow = T),
    matrix(rep(c(0, 1), 83), ncol = 2, byrow = T),
    matrix(rep(c(1, 0), 240), ncol = 2, byrow = T),
    matrix(rep(c(0, 0), 1349), ncol = 2, byrow = T)
) # 2 ratings for each of n patients
norm <- radiologists[, 1]
nonnorm <- radiologists[, 2]
mat <- table(norm, nonnorm)
mat0 <- mat
mat <- rbind(mat, apply(mat, 2, sum))
mat <- data.frame(cbind(mat, apply(mat, 1, sum)))
mat <- cbind(rep(NA, 3), mat)
mat <- rbind(c("Radiologist A", "Pneumonia", "No pneumonia", "Total"), mat)
mat[2:4, 1] <- c("Pneumonia", "No pneumonia", "Total")
dimnames(mat)[[2]] <- c("", "Radiologist B", "", "")
xtab2 <- xtable(mat, align = "cc|cc|c", caption = "", label = "tab: kappa1")
...
data <- data.frame(
  "Radiologist A" = c("**Pneumonia**", "**No pneumonia**", "**Total**"),
  "Pneumonia" = c(176, 83, 259),
  "No pneumonia" = c(240, 1349, 1589),
  "Total" = c(416, 1432, 1848),
  check.names = FALSE
)

# Print the table using kable
kable(data, format = "html", row.names = FALSE, booktabs = TRUE, 
      caption = "Agreement of binary classification of patients between Radiologist A and B.")%>%
  kable_styling(full_width = FALSE) %>%
  add_header_above(c(" " = 1, "Radiologist B" = 2, " " = 1))
```
...
Table \@ref(tab:exbinary) is $259 \cdot 416 / 1848 = 58.3$. 
...
data <- data.frame(
  "Radiologist A" = c("**Pneumonia**", "**No pneumonia**", "**Total**"),
  "Pneumonia" = c(round(mat.exp[2,2], 1), round(mat.exp[2,1], 1), "259"),
  "No pneumonia" = c(round(mat.exp[1,2], 1), round(mat.exp[1,1], 1), "1589"),
  "Total" = c(416, 1432 , 1848),
  check.names = FALSE
)
...
:::{.definition}
  ...
  standardised with the maximal possible value $1-\pexp$:
:::
...
Note that $\kappa \leq \pobs$ is always true. When $\kappa = 1$, 
there is perfect agreement ($\pobs=1$). When $\kappa = 0$, the agreement is not larger than expected by chance ($\pobs=\pexp$). The smallest possible value of $\kappa$ is $-\pexp/(1-\pexp)$. A negative $\kappa$ means that the agreement is worse than by chance. 

In Example \@ref(exm:Hazir) we obtain
...
kable(kappa_table, format = "html", caption = "Classification of Cohen's Kappa [@altman1991 and @bland].")
```
...
Assume the two observers have the same chance of a
correct assessment, we have the following figure.

```{r kappa-dep, fig.cap = "Dependence of $\\kappa$ on the true proportion of positive subjects.", echo = FALSE, warning=FALSE}
k <- function(p, q){
    num <- p*(1-p)
...
for(i in 1:length(q)) 
    kappa[,i] <- k(p1, q[i])
par(las=1)
matplot(p1, kappa, type="l", xlim=c(0,1.5),ylim=c(0,1), xlab="True proportion of 'Yes'", ylab="Expected Kappa", lty=1, lwd=2)
...
legend("right", lty=1, col=rev(1:6), legend=paste(as.character(rev(q*100)), "% correct", seq=""), bg="white", cex=1, lwd=2)
``` 


```{r kappa-extreme, results = "asis", echo = F, message = FALSE, warning=FALSE}
mat <- rbind(
...
knitr::kable(mat, format = "html",
      caption = "An extreme example for Kappa interpretation.", 
      row.names = FALSE, 
      escape = FALSE) %>%   
  kable_styling(full_width = FALSE)
```

This case shows that $\kappa$ can be small even when you have a good agreement.
...
## names <- c("normaliser", "non-normaliser")
...
pneumonia <- matrix(c(176, 240, 83, 1349), byrow=T, ncol=2, nrow=2)
pneumonia <- as.table(pneumonia, dnn=c("Radiologist A","Radiologist B"))
names <- c("Pneumonia", "No pneumonia")
colnames(pneumonia) <- names
rownames(pneumonia) <- names

## ratings for each of n patients

pneumonia.ind <- rbind(matrix(rep(c("Pneumonia", "Pneumonia"), 176), ncol = 2, byrow = T),
                      matrix(rep(c("No Pneumonia", "Pneumonia"), 240), ncol = 2, byrow = T),
                      matrix(rep(c("Pneumonia", "No Pneumonia"), 83), ncol = 2, byrow = T),
                      matrix(rep(c("No Pneumonia", "No Pneumonia"), 1349), ncol = 2, byrow = T))
#pneumonia.ind <- cbind(c(1:nrow(pneumonia.ind)), pneumonia.ind)
pneumonia.ind <- as.data.frame(pneumonia.ind)
colnames(pneumonia.ind) <- c("RadiologistA", "RadiologistB")
rownames(pneumonia.ind) <- NULL
...
with each row representing the measurements from one child:
```{r format2, echo=T}
print(head(pneumonia.ind, 3))
print(table(pneumonia.ind))
``` 
...
```{r echo=TRUE}
pobsCI <- wilson(x=sum(diag(pneumonia)), n=sum(pneumonia))
pexp <- sum(diag(chisq.test(pneumonia)$expected))/sum(pneumonia)
## substitution method
...
res <- biostatUZH::confIntKappa(pneumonia.ind, type="Cohen", m=10000)

...
The expected value of $\kappa$ also depends on the true proportion of subjects in the two
categories. Figure \@ref(fig:kappa-dep) shows the expected value of $\kappa$ as a function of the
true proportion of "Yes" subjects for different probabilities of a correct assessment by the two classifiers. 
We can see that expected $\kappa$ drops for extreme proportions close to zero or one. 


In Table \@ref(tab:exbinary),
...
(5/94 resp. 6/94).
 Figure \@ref(fig:kappa-dep) shows that the expected value of $\kappa$ also depends on the marginal distribution of positive
(respectively negative) subjects. It can be quite low, even if the probability of a correct assessment is quite large. 



```{r kappa-dep, fig.cap = "Dependence of expected $\\kappa$ on the true proportion of positive subjects.", echo = FALSE, warning=FALSE}
k <- function(p, q){
    num <- p*(1-p)

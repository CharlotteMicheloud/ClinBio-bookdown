...
A *crossover trial* compares the outcome of a patient when given 
...
There are two treatment periods, and patients are randomised into two
...
had been allocated to group AB. For example, to study the
...
all patients were treated in the order A-B, then any difference in
...
kable(period_data, escape = FALSE, booktabs = TRUE,
      caption = "Expected mean outcomes for the AB/BA design with (continuous) data.")
```
...
if we allow for *patient-specific effects* $\xi_i$, since the $\xi_i$'s cancel when we calculate patient-specific differences 
$d_{i} = x_{i1} - x_{i2}$. The below table illustrates that between-patient variation is eliminated in the standard analysis of crossover trials.
...
kable(period_data, escape = FALSE, booktabs = TRUE,
      caption = "Expected mean outcomes incorporating patient-specific effects ($\\xi_i$).")
```
...
## The issue of carryover {#testcarryover}
...
A statistical approach to handle carryover has been suggested: first, test for carryover, and if significant, compare only data from period 1, otherwise, analyse data from period 1 and 2 jointly assuming there is no carryover effect. We emphasise that this is generally not recommended due to lack of power and other problems. 

The recommended approach is not to use a crossover design when there is a possibility of a carryover effect. 
...
kable(period_data, escape = FALSE, booktabs = TRUE, label = NA, 
      caption = "Expected mean outcomes incorporating a carryover effect ($\\gamma$).") %>%
...
# Printing the table using kable
kable(period_data, escape = FALSE, booktabs = TRUE, label = NA,
      caption = "Fitted values for saturated model.") %>%
  kable_styling(latex_options = c("hold_position", "striped"))
```
...
We now discuss sample size calculation for an AB/BA crossover design
...
The standard parallel group design thus requires
  \[
    ...
    \]
patients per group. Fewer patients are needed in a crossover trial, 
...
The results of a crossover trial with binary outcome can be summarised as in 
the following table:
...
kable(
  ...
  format = "html", 
  caption = "Summary of a crossover trial with binary outcomes."
) %>%
...
<!-- or a generalised linear mixed model. -->


### Naive analysis

A naive way to analyse such data would be to compare the treatment 
groups using McNemar's test for binary paired data, ignoring the 
...
It is also possible to perform McNemar test using the `R` function 
`mcnemar.test()`: 
```{r echo=FALSE}
...
slightly different test statistic (with or without continuity correction).
...
This approach is known as the Mainland-Gart test, see @Senn2 (Section 4.4.2). 
Note the additional exponent and factor $1/2$ in $\widehat{\OR}$ and $\SE(\hat \beta)$ respectively, as in the analysis of a continuous outcome. 
...
### Analysis with generalised linear mixed models
We may also use a generalised linear mixed model (but results may depend on the choice of the integration parameter `nAGQ`):
